...
| `<storageBasePath>/model/TTS` | 默认 TTS 模型缓存目录（旧版 `<storageBasePath>/tts-models` 仅作迁移兼容） |
| `<storageBasePath>/TTS/runtime` | TTS sidecar 运行数据、`tts.sqlite`、生成音频、依赖 hash marker 和 runtime config（旧版 `<userData>/tts-runtime` 仅作迁移兼容） |
...
默认 `video:daojie:chapter001` 自动链不会注入 `MANYING_TTS_USE_HTTP=1`，因此不会经过这条 HTTP-TTS 直跑分支。直跑脚本会按 `MYSTUDIO_STORAGE_BASE_PATH`、`<userData>/storage-config.json`、macOS development fallback 的顺序解析 managed 存储根；其中 `python/` 属于 `<storageBasePath>`，TTS 模型位于 `<storageBasePath>/model/TTS/`，运行数据位于 `<storageBasePath>/TTS/runtime/`。
