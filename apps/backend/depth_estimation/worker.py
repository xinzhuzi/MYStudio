#!/usr/bin/env python3
"""Depth estimation worker CLI — mirrors the video_use worker pattern.

Usage:
  python -m depth_estimation.worker --probe
  python -m depth_estimation.worker --version
  python -m depth_estimation.worker --run --input <request.json> --output <artifact.json>
"""
...
def _blocked(request: dict[str, Any], code: str, message: str) -> dict[str, Any]:
    return {
        ...
        "status": "blocked",
        "model": str(request.get("model", "depth-anything-v2-small")),
        "inputSha256": "0" * 64,
        "outputSha256": "0" * 64,
        "outputPath": "",
        "width": 0,
...
def _run(input_path: str, output_path: str) -> dict[str, Any]:
    ...
    output_depth = str(request.get("outputDepthPath", ""))

    if not input_image or not output_depth:
        return _blocked(request, "missing-paths", "inputImagePath 和 outputDepthPath 必须提供")

    try:
        result = estimate_depth(input_image, output_depth, model)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MYStudio depth estimation worker")
    ...
    group.add_argument("--run", action="store_true", help="Run depth estimation")
