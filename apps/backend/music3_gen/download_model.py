#!/usr/bin/env python3
"""Explicit Music3 model downloader. Usage:
  python -m music3_gen.download_model --model minimax-music3-mlx --progress <progress.json>
"""
...
def download_model(model_name: str, progress_path: Path) -> int:
    ...
    try:
        ...
        monitor.start()
        try:
            # 该仓库仅存在于 HF(社区自含移植);ModelScope 未镜像,直接 HF 端点。
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MYStudio Music3 model downloader")
