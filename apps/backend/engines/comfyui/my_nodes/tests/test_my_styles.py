...
def test_real_tree_run_gongbi_assembles_manual_anchors(monkeypatch):
    ...
    assert "一幅2D中国传统工笔画，传统设色；" in positive  # 09-16 全角化(用户 GPT-image-2 实证文本)
...
FROZEN_GONGBI_POSITIVE = '| 质量锚定 | 最佳质量、杰作、高细节；一幅2D中国传统工笔画，传统设色；精谨线描，细腻墨线，线条细而稳，勾勒精确有韵律；层层设色，分染罩染提染，矿物颜料层染过渡细腻；用色以石青、石绿、朱砂、赭石、花青为主，雅致低饱和，强色只作局部点缀；人物为东方古典造型，身姿修长端雅，五官清秀，神情含蓄；装饰有序，传统纹样精致不堆砌；非对称平衡构图，大面积留白，主体精致与留白安静相衬；画面呈现细腻的传统绘画质感与手绘颜料表面，均匀平面柔光，无投影；宁静诗意，古雅氛围，是一幅完成度高的工笔画。 |'
...
def test_gongbi_base_prompt_frozen(monkeypatch):
    ...
    assert "2D中国传统工笔画" in pos and "精谨线描" in pos and "非对称平衡构图" in pos
