...
def test_real_tree_run_gongbi_assembles_manual_anchors(monkeypatch):
    ...
    assert "一幅2D中国传统工笔画，矿物颜料设色；" in positive  # 09-16 全角化(用户 GPT-image-2 实证文本)
...
FROZEN_GONGBI_POSITIVE = '| 质量锚定 | 最佳质量、杰作、高细节；一幅2D中国传统工笔画，矿物颜料设色；线描分工：面部与发丝以游丝细线逐根勾勒，衣纹以铁线描勾勒，线条细而稳；用色以石青、石绿、朱砂、赭石、花青为主，低饱和，强色只作局部点缀；人物造型修长端雅，五官清秀，神情含蓄；纹样精致不堆砌，仅作衣缘局部点缀；非对称平衡构图，大面积留白；画面呈现细腻的手绘颜料质感，均匀平面柔光，无投影；宁静诗意，完成度高的工笔画。 |'
...
def test_gongbi_base_prompt_frozen(monkeypatch):
    ...
    assert "2D中国传统工笔画" in pos and "线描分工" in pos and "非对称平衡构图" in pos
