...
// 「漫影账户」并点击转跳该网址(卡片不隐藏)。
(function myLoginCloak() {
  if (window.__mySignInCloak) return;
  window.__mySignInCloak = true;
  ...
  // 双读注入变量:新名 MY_* 优先,旧名 MANYING_* 兼容(装机旧 asar 注入期)
  const ACCOUNT_URL = (window.MY_ACCOUNT_URL || window.MANYING_ACCOUNT_URL || "").trim();
