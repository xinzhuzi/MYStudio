...
// 载荷=生成器写进 properties.myStage(v2:全量正文行/缩略图 tiles/表行/
...
app.registerExtension({
  name: "my.stage.render",
  async beforeRegisterNodeDef(nodeType, nodeData) {
    // 旧名 ManyingStage=存量工作流兼容(09-14 manying→my 改名)
...
const payload = this.properties?.myStage ?? this.properties?.manyingStage;
      ...
      try {
        ...
        if (actions.length > 0) {
          ...
          for (const action of actions) {
            ...
            const flash = this.__myActionFlash
              && this.__myActionFlash.kind === action.kind
              && Date.now() < this.__myActionFlash.until;
    ...
    nodeType.prototype.onMouseDown = function (pos) {
      const rects = this.__myActionRects || [];
