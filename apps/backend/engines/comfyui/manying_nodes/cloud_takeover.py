...
def apply_cloud_takeover() -> bool:
    ...
    if original is None or getattr(original, "_manying_takeover", False):
        return False

    def _manying_get_auth_header(node_cls):  # noqa: ANN001 — 与上游签名对齐
        # 漫影网关单钥:无论前端有没有 comfy.org 会话,凭据恒=漫影令牌
        # (登录入口由此在功能层失效:杂散 comfy.org token 永不被采用)。
        return {"Authorization": f"Bearer {token}"}

    _manying_get_auth_header._manying_takeover = True
    _helpers.get_auth_header = _manying_get_auth_header
    return True
