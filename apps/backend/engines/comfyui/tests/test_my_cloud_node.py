...
from engines.comfyui import my_nodes as my_nodes_pkg
from engines.comfyui.my_nodes.bridge import settings
from engines.comfyui.my_nodes.nodes import my_cloud_image as node_mod


def test_node_registered_with_display_name():
    assert my_nodes_pkg.NODE_CLASS_MAPPINGS["MyCloudImage"] is node_mod.MyCloudImage
    assert my_nodes_pkg.NODE_DISPLAY_NAME_MAPPINGS["MyCloudImage"] == "漫影 云端生图"


def test_input_types_shape():
    spec = node_mod.MyCloudImage.INPUT_TYPES()
    ...
    assert node_mod.MyCloudImage.RETURN_TYPES == ("IMAGE",)
