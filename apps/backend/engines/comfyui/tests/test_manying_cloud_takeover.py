...
from engines.comfyui.manying_nodes import cloud_takeover
...
def test_no_comfy_api_nodes_module_is_silent(monkeypatch):
    monkeypatch.setenv(cloud_takeover.TOKEN_ENV, "sk-manying")
...
def test_takeover_swaps_auth_header_everywhere(monkeypatch):
    ...
    assert fake.get_auth_header(object()) == {"Authorization": "Bearer sk-manying"}
    # client.py/nodes_sonilo 按名引 get_comfy_api_headers 的路径同样生效
    headers = fake.get_comfy_api_headers(object())
    assert headers["Authorization"] == "Bearer sk-manying"
