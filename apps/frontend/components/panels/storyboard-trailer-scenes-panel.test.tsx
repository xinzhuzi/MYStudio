...
} from "./storyboard-trailer-scenes-panel";
...
describe("StoryboardTrailerScenesPanel", () => {
  it("shows the trailer empty state without toolbar or actions", () => {
    render(<StoryboardTrailerScenesPanel {...createProps({ trailerScenes: [] })} />);
  ...
  it("disables destructive and generation actions while generation is active", () => {
    render(<StoryboardTrailerScenesPanel {...createProps({ isGenerating: true })} />);
  ...
  it("keeps the S-Class compatibility export wired to the shared panel", () => {
    render(<SClassTrailerScenesPanel {...createProps()} />);

    expect(screen.getByTestId("storyboard-config-toolbar")).toBeTruthy();
    expect(screen.getByText("分镜卡片 0")).toBeTruthy();
  });
});
