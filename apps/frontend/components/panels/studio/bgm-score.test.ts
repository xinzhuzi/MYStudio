...
/** 桥模板最小镜(出谱图 15/23/14;BGM 渲染图只列注入涉及的 22/23)。 */
...
function renderWorkflowText(): string {
  return bridgeWorkflowText({
    "22": { class_type: "YuE2GenerateMusic", inputs: { style: "orig", lyrics: "orig", abc: ["14", 0], seed: 42 } },
    "23": { class_type: "YuE2GenerateABC", inputs: { style: "orig", lyrics: "orig", seed: 42 } },
  });
}
...
describe("buildScoreInputs / buildRenderByScoreInputs(注入面·纯函数)", () => {
  ...
  it("按谱渲染注入:BGM 单发双节点口径 + 22.abc 直喂(字面量替换链接位)", () => {
...
describe("generateBgmScore / renderBgmByScore(编排器·全 mock)", () => {
  ...
  it("按谱渲染:注入含 22.abc;首音频产物回带;超时档 1200s", async () => {
    ...
    const payload = execute.mock.calls[0][0];
    expect(payload.inputs.strings["22.abc"]).toBe(ABC_SAMPLE);
    expect(payload.inputs.strings["22.style"]).toBe("guzheng");
