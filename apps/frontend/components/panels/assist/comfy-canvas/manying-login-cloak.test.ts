...
// manying_login_cloak.js 在 jsdom 里实跑——测的是「随引擎分发的同一份代码」,
...
const cloakSource = readFileSync(
  path.resolve(process.cwd(), "backend/engines/comfyui/manying_nodes/web/manying_login_cloak.js"),
  "utf8",
);
...
beforeEach(() => {
  ...
  delete (window as { __manyingSignInCloak?: boolean }).__manyingSignInCloak;
  delete (window as { MANYING_ACCOUNT_URL?: string }).MANYING_ACCOUNT_URL;
});

describe("manyingLoginCloak(引擎扩展真源,二轮精确化)", () => {
