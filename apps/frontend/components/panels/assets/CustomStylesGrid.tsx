...
let _customManualsCache: StudioVisualManualSummary[] | null = null;
...
export function CustomStylesGrid() {
  ...
  const loadManuals = async (force = false) => {
    ...
    if (!force && _customManualsCache) {
      setManuals(getCustomVisualManuals(_customManualsCache));
      setAllManuals(_customManualsCache);
      return;
    }
    setIsLoading(true);
    try {
