...
// 全局缓存，避免重复加载
let _manualsCache: StudioVisualManualSummary[] | null = null;
...
export function DefaultStylesGrid() {
  ...
  const loadManuals = async (force = false) => {
    ...
    if (!force && _manualsCache) {
      setManuals(_manualsCache);
      setExpandedManualCategories(new Set(groupDefaultVisualManuals(_manualsCache).map((group) => group.id)));
      return;
    }
    setIsLoading(true);
    try {
  ...
  useEffect(() => {
    // 延迟 100ms 加载，避免切换时阻塞渲染
    const timer = setTimeout(() => void loadManuals(), 100);
    ...
    <div className="flex h-full flex-col">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => loadManuals(true)} disabled={isLoading}>
