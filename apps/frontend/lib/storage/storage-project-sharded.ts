...
import { isSafeProjectId, getActiveProjectId, type ProjectFilesTextBridge } from "./project-storage";
...
import { STUDIO_WORKFLOW_SHARD_DIR, StudioWorkflowDomainGeneration, StudioWorkflowShardManifest, md5Utf8, mergeStudioWorkflowShards, parseStudioWorkflowShardManifest, planStudioWorkflowShards } from "./studio-workflow-shards";
import { useProjectStore } from "@/stores/project/project-store";
import type { StateStorage } from "zustand/middleware";

/**
 * 工作室工作流分片存储引擎——createStudioWorkflowShardedStorage 与分片键/写串行/代际抽取私有工具及 README 守护族。深网专批矩阵驱动,体逐字保留。
 */
...
export async function ensureReadmeMatches(
  pid: string,
  relativePath: string,
  template: string,
  label: string,
 ...
 */
export async function ensureStudioWorkflowReadme(pid: string): Promise<void> {
 ...
 */
export async function ensureProjectRootReadme(pid: string): Promise<void> {
 ...
 */
export async function ensureBackupsReadme(pid: string): Promise<void> {
