...
import type { AICharacter, CharacterBibleLike } from '@/lib/ai/core';
import { PromptCompiler } from '@/lib/ai/core/services/prompt-compiler';
import { createWorkerApi } from './ai-worker-api';
import { handleExecuteScreenplayVideos as handleExecuteScreenplayVideosImpl, executeSceneInternal, generateSceneImageOnly } from './worker-screenplay-videos-handler';
  ...
  for (let i = 0; i < scenes.length; i += concurrency) {
    ...
    await Promise.allSettled(
      batch.map(async (scene) => {
        try {
          await executeSceneInternal(run, api, screenplay.id, scene, extendedConfig, screenplay.characterBible, characterReferenceImages, { isCancelled, reportSceneFailed, reportSceneProgress, getBibleCharacters, postEvent });
          completedCount++;
        } catch (error) {
          failedCount++;
          const err = error as Error;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  for (let i = 0; i < scenes.length; i += concurrency) {
    ...
    await Promise.allSettled(
      batch.map(async (scene) => {
        try {
          await generateSceneImageOnly(run, api, screenplay.id, scene, extendedConfig, screenplay.characterBible, characterReferenceImages, { isCancelled, reportSceneFailed, reportSceneProgress, getBibleCharacters, postEvent });
          completedCount++;
        } catch (error) {
          failedCount++;
          const err = error as Error;
...
async function handleExecuteScreenplayVideos(command: ExecuteScreenplayVideosCommand): Promise<void> {
  await handleExecuteScreenplayVideosImpl(command, {
    beginRun,
    isCancelled,
    postEvent,
    setApiBaseUrl: (url: string) => { apiBaseUrl = url; },
    reportSceneProgress,
    reportSceneFailed,
    getBibleCharacters,
  });
