...
export function registerStudioRenderIpcHandlers({
  ...
  let timelineRenderHost: ReturnType<typeof createTimelineRenderHost> | null = null;
  const getTimelineRenderHost = () => {
    if (timelineRenderHost) return timelineRenderHost;
    const emitProgress = (progress: TimelineRenderProgress) => {
  ...
  ipcMain.handle("studio-timeline-render", async (_event, request: unknown) => getTimelineRenderHost().render(request));
  ipcMain.handle("studio-timeline-render-cancel", async (_event, jobId: string) => getTimelineRenderHost().cancel(jobId));
