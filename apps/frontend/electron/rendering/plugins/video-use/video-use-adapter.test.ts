...
describe("video-use adapter", () => {
  ...
  it("passes alignment to the formal worker and preserves its blocked code", async () => {
    ...
    expect(call).toBe(2);
    expect(environments[0]?.MANYING_TTS_MODELS_DIR).toBe(path.join(root, "comfyui", "models", "TTS"));
    expect(environments[0]?.VOICEBOX_MODELS_DIR).toBe(path.join(root, "comfyui", "models", "TTS"));
  });
});
