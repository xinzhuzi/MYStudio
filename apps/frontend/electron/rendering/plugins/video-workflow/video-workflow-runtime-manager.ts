...
export function createVideoWorkflowRuntimeManager(
  ...
  const prepareVideoUse = async (): Promise<VideoWorkflowRuntimePaths> => {
    ...
    if (!isFile(paths.pythonExecutable)) throw new Error("managed Python executable 不存在，请先在 Python 配置页完成准备");
    if (!isExecutableFile(paths.pythonExecutable, platform)) throw new Error("managed Python executable 不可执行，请在 Python 配置页修复运行时");
