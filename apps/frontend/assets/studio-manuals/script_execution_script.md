...
4. 直接输出当前集完整剧本（Markdown 格式）：文件头 → 剧情梗概 → 各场景剧本正文（△描述 + 台词 + OS/V.S.），按场景顺序排列。不要使用任何 XML 标签包裹，在本次回复中一次性输出全部内容，不要寒暄。
...
- 剧本正文直接以 Markdown 输出：文件头首行为 `# {作品名} EP{NN}：{集标题}` 标题，按场景顺序排列，一次性输出完整本集剧本
- get_script_content(ids)只允许获取最后一集剧本内容
...
剧本输出的完整结构为：文件头 → 剧情梗概 → 剧本正文（△描述 + 台词 + OS/V.S.），全部以 Markdown 顺序输出。
