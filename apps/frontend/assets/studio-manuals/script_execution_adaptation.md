...
4. 直接输出完整改编策略（Markdown 格式），依次包含：核心改编原则、主要删除/压缩决策、世界观呈现策略、核心视觉符号、色调对比体系、镜头节奏策略。不要使用任何 XML 标签包裹，在本次回复中一次性输出全部内容，不要寒暄。
...
提炼 4-8 个贯穿本集的关键视觉符号（参考所选导演/视觉手册的画面语言），每个一行：

| 符号 | 所在幕 | 呈现方式 | 情绪功能 |
|------|--------|----------|----------|
| {符号名} | {第几幕} | {如何在画面中呈现，可拍摄} | {承担的情绪作用} |
...
| 幕 | 色调 | 核心色彩 | 转场衔接方式 |
|------|------|----------|-------------|
| {第N幕·小标题} | {冷/暖/中性} | {具体色彩词} | {与下一幕的色调过渡方式} |

**核心原则：** {说明三幕色调如何构成情绪曲线，如"冷→暖→更冷"的 V 型}

### 镜头节奏策略

**基本原则：** {一句话，如 固定机位为主、运动镜头为情绪服务}

| 场景 | 机位策略 | 节奏 | 目的 |
|------|----------|------|------|
| {场景} | {固定/跟拍/快切等} | {每镜头秒数} | {要达到的效果} |

如有蒙太奇段落，补充：
**蒙太奇快切节奏（{段落名}专用）：** {快切的节拍设计与时长控制}
