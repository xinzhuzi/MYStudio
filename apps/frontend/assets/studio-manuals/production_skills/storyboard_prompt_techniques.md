...
> 分镜帧图会进入「深度估计 → 2.5D 相机运镜」管线。写分镜提示词时必须遵循纵深构图规则，完整 token 与规则见同目录 **`depth_friendly_3d.md`**（唯一真相源，生图时代码自动追加其标记块 token）。

**本技法核心条款（写提示词时执行）**：

| 编号 | 规则 |
|------|------|
| D1 | 构图必须三层分离：前景(遮挡元素) / 中景(主体) / 背景(环境)，提示词中三层各自可见 |
| D2 | 优先安排近景遮挡元素（伸入画框的树枝/门框/廊柱/人物肩背）— 视差感最大放大器 |
| D3 | 有条件时纳入透视引导线（道路/走廊/街道/河流延伸至消失点） |
| D4 | 远景淡化（大气透视），主体保持锐利 — 与「画质降级禁用词」协调，禁止全图雾化 |
| D5 | **景别虚化覆盖**：进入 2.5D 管线的分镜，景别词库中的「背景虚化/浅景深/虚化背景/背景深度虚化」一律改写为「背景略淡化、主体与背景均清晰」— **景深由渲染阶段按深度图施加，源图虚化会毁掉深度边缘** |
| D6 | 主体轮廓与背景明暗/色彩分明，避免主体融入背景 |
| D7 | 禁止：正面平面化构图、海报式平涂背景、大面积无结构空墙、镜面/玻璃/半透明主体 |

**风格适配**：视觉风格手册可在自身 `art_storyboard_video.md` 定义风格化纵深条款（如水墨版「远山淡墨」），优先于本通用规则。
