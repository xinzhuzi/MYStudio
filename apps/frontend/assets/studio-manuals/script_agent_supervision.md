...
| 剧本审核、审核剧本、剧本、review script | 剧本 → 执行「剧本审核」 |
...
1. **产出物为准**：审核依据以实际产出物为准——可调用工具读取；**若审核对象已随任务直接提供，则基于所提供内容审核**，不得凭记忆臆测
...
1. **格式规范**：场景标题（时间/地点/内外景）、人物、台词、动作描述、OS/VO/字幕标注是否齐全规范，可直接用于拍摄
2. **可拍摄性**：是否存在不可拍摄的抽象/心理描写未转化为可视画面或 OS/VO；场景是否具体可执行
3. **台词质量**：是否口语化、精简有力；有无文言/生僻冷词；是否符合人物身份与短剧称谓规范（禁"市长""县长"等）
4. **开场黄金3秒**：第1场是否强冲突/强情绪、一秒入坑；是否拖沓铺垫
5. **节奏与时长**：每场是否有效推进（无废戏）；总时长是否贴合该集骨架设定的单集时长
6. **情绪与钩子兑现**：是否落实骨架该集的情绪点（爽/虐/甜）与集末钩子；付费点对应场景是否够"宏大/紧急/围观"、卡点是否有力
7. **落实上游**：是否落实改编策略的原则与删减、骨架的分集结构/付费点/信息差；有无擅自新增主线或保留已删内容
8. **人物一致性**：人物言行是否符合设定与弧光；信息差视角是否与骨架标注一致
...
1. 读取随任务提供的【剧本】（审核主体）
2. 读取随任务提供的【故事骨架】【改编策略】作为对照基准
3. 从【项目配置】读取：单集时长、平台规格、付费策略

### 审核维度

| 审核项 | 标准 | 严重程度 |
|--------|------|----------|
| 可拍摄性 | 无未转化的抽象/心理描写；场景具体可执行（→ Skills 四-2） | 严重 |
| 开场黄金3秒 | 第1场强冲突/强情绪、一秒入坑，无拖沓铺垫（→ Skills 四-4） | 严重 |
| 情绪与钩子兑现 | 落实骨架该集情绪点与集末钩子；付费点场景宏大/紧急、卡点有力（→ Skills 四-6） | 严重 |
| 落实上游 | 落实改编策略原则与删减、骨架结构/付费点/信息差；无擅自新增主线或保留已删内容（→ Skills 四-7） | 严重 |
| 格式规范 | 场景标题/人物/台词/动作/OS/VO/字幕齐全规范，可直接拍摄（→ Skills 四-1） | 中等 |
| 台词质量 | 口语化、精简；无文言/生僻词；称谓符合短剧规范（→ Skills 四-3） | 中等 |
| 节奏与时长 | 每场有效推进无废戏；时长贴合该集骨架设定（→ Skills 四-5） | 中等 |
| 人物一致性 | 言行符合设定与弧光；信息差视角与骨架标注一致（→ Skills 四-8） | 中等 |

### 跨阶段一致性检查

剧本需与故事骨架、改编策略进行一致性校验：

- **结构落实**：剧本是否覆盖骨架为该集设定的关键节点（开场钩子、付费点、卡点、集末钩子），无遗漏或错位
- **删减一致**：改编策略中标注删除的内容，剧本不得出现；标注保留的情绪点/打脸/拉扯，剧本须呈现
- **情绪基调一致**：剧本实际情绪走向与骨架/改编策略确定的基调一致，无中途偏离
