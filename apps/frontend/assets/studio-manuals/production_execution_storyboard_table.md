...
2.  **每个片段 ≤15 秒**，单片段台词字数超载时，拆分为多个片段。若所选导演手册给定长镜基线（如 8 分钟长镜制每镜 8~15s），按手册镜长执行，不因台词密度提前碎切。

3.  **长台词/长 VO 拆镜（手册基准优先）**：默认超过 20 字拆多个连续镜头；但若所选导演手册为长镜制（一镜承载 2~4 句），优先按手册在镜内以表情微变/肢体演进/缓推运镜承载，仅语义确需切换时拆镜。台词零删改不受影响。台词可以第一镜完整写出。跨镜可切到他人反应镜（画面是听者，声音继续是说话人）。若语义不可切必须单镜呈现，则用**表情的微妙变化/肢体动作的持续演进/运镜的缓慢推动**填满时间，禁止单镜固定。

4.  **台词零删改铁律**：剧本中所有引号内台词、VO、系统播报、面板文字必须 100% 逐字搬运。禁止合并、禁止精简、禁止省略修饰词。分镜师只设计画面，不做台词二次创作。

5.  **台词时间分配**：根据情绪和语气分配，不平均切分；默认按 4字/秒 估算，若所选导演手册另有语速基准（如 8 分钟制 150字/分钟≈2.5字/秒 含呼吸留白），按手册执行。
...
13. **逐镜出镜语义不可省略**：每一镜必须在表格的 `出镜语义JSON` 列写出本镜的 `sceneViewpointId`、真正可见的人物和真正可见的道具，而不是复用场头的全部参演角色或片段资产。每个入画角色都必须有名称、九宫格站位、朝向、入镜动作和出镜动作；每个可见道具都必须有名称、位置和本镜状态；没有任何人物或道具时分别写空数组。没有人物时只能明确写 `personFree:true`，禁止用空数组、`无` 或省略字段来猜测空镜。角色和道具名称必须来自本镜 `关联资产名称` 列，不得以台词说话人、场头演员表或未出镜资产推断入画。

14. **逐镜转场意图必写**：每一镜的 `出镜语义JSON` 必须携带 `transitionToNext` 字段——它是你作为分镜师对"本镜切向下一镜"这个边界的显式节奏决策。`styleWord` 只能从词表选：`同场景硬切`（硬切是一种正当的节奏效果，动作连贯/快节奏时首选）、`水墨晕染`、`灵气色彩`、`境界跃迁`、`四季流转`、`剑痕`、`血祭`、`梦境`、`空镜呼吸`；`moodWord` 选填（战斗/回忆/天道/阴谋/日常/承接），供特效层使用。判断标准：动作连续或节奏要快→硬切；情绪/时空转换→选对应风格词。最后一镜也必须写（指向下一场首镜）。
...
> **职责边界**：本节写的是**叙事连贯技巧**。机器转场的逐镜意图由本技能输出——写在每镜 `出镜语义JSON` 的 `transitionToNext` 字段（见铁律 14，风格词从词表选，硬切是正当选择）；导演规划⑥段只提供场级基调，经 video-use 决策层汇总裁决落到成片。不要另加表格列，也不要发明词表外的风格词。
...
- **逐场注意事项**：把该场列出的 `情感砸点 / 一致性锚点 / 空间距离 / 易错提示` 逐条落实到具体镜头设计中（情感砸点 → 给到位的景别 / 运镜；一致性锚点 → 跨镜画面内容连续；空间距离 → 站位与景别）。`环境音` 仅作氛围理解参考；最终每镜的音效落在表格的 `音效` 列。
...
每个场次以一行**场头**开始，其后是该场若干**片段**；每镜一行的**15 列全格式**表格落在片段之内：

**场头**：`## 场N：场景名 ｜ 参演角色：角色A、角色B、…`
- N 从 1 起，对应剧本 / 分场汇总表的场次顺序与场景名。
- 参演角色 = 该场全部出场角色（含仅局部 / 背影 / 虚焦可见者），按出场顺序列出；纯空镜场写「参演角色：无」。

**片段**：`### 片段X（约10s）`，只做结构分组，**不再写片段级「引用资产名称 / 引用资产ID」行**——资产逐镜写在表格列里。

**15 列固定表头**（顺序与列数不得增删改，供机器校验）：

```
| 序号 | 画面描述 | 场景 | 关联资产名称 | 时长 | 景别 | 运镜 | 角色动作 | 朝向 | 空间关系 | 情绪 | 台词 | 音效 | 关联资产ID | 出镜语义JSON |
|------|------|------|------|------|------|------|------|------|------|------|------|------|------|------|
| 1 | 西瓜筐被一脚踢飞腾空，筐内西瓜飞溅而出，西瓜在林志强脚边砸地炸裂，红瓤四溅，黄土扬起。 | 道口镇院子 | [林志强, 西瓜筐] | 5 | 近景 | 缓推 | 林志强侧身避开碎瓤，右脚钉在原地未退 | 林志强正面三分之四朝左 | 西瓜筐左前景，林志强右中格，两者斜向对峙 | 突发紧张 | 旁白：西瓜在林志强脚边砸裂。 | 音效：西瓜筐翻滚撞地声、西瓜炸裂闷响、瓜瓤溅落声 | [char_101, prop_301] | {"sceneViewpointId":"yard-gate-axis","personFree":false,"visibleCharacters":[{"name":"林志强","position":"右中格","orientation":"正面三分之四朝左","actionIn":"右脚停在落点旁，低头看向飞来的西瓜","actionOut":"侧身避开碎瓤，右脚仍停在原地"}],"visibleProps":[{"name":"西瓜筐","position":"左前景","state":"翻起并倾倒"}],"actionIn":"西瓜筐从左前景翻起，林志强位于右中格","actionOut":"碎瓤落到林志强右脚边","transitionToNext":{"styleWord":"同场景硬切","moodWord":"战斗"}} |
```

**各列填写要求**（新增列禁止占位）：
- **场景**：本镜所在场景的资产名称（来自 `assets`，逐镜填写，与场头一致）。
- **关联资产名称 / 关联资产ID**：本镜真正出镜或被引用的资产（逐镜精确到本镜可见，不得照抄场头参演角色全表）。
- **角色动作**：本镜主体的动作要点——讲「谁在做什么」，**禁止照抄画面描述**（画面描述讲「画面长什么样」）。
- **朝向**：出镜主体相对镜头的朝向关系（正面 / 侧面 / 背面、三分之几），**禁止写"—"**。
- **空间关系**：主体之间的九宫格站位与前后景层次，**禁止写"—"**。
- **情绪**：本镜情绪基调（与导演规划的情绪浓度对齐）。

`出镜语义JSON` 必须是单行有效 JSON，且不得包含竖线 `|`。固定结构如下；`sceneViewpointId` 指向本镜主场景的已声明视角版本；`personFree:true` 时 `visibleCharacters` 必须为空，但 `visibleProps`、`actionIn` 与 `actionOut` 仍须按本镜实际状态填写：

```json
{
  "sceneViewpointId": "本镜已声明的场景视角版本",
  "personFree": false,
  "visibleCharacters": [
    {
      "name": "资产中的角色名",
  ...
  "actionIn": "整镜起始构图与动作状态",
  "actionOut": "整镜结束构图与动作状态"
}
```
...
10. **出镜语义可执行**：每一镜均有有效 `出镜语义JSON`；可见角色全部在该镜 `关联资产名称` 内；角色未入画时不得列入 `visibleCharacters`，也不得用场头参演角色替代本镜语义。
11. **15 列全格式**：表头固定为 15 列且顺序不变；`朝向`、`空间关系` 禁止写"—"或留空；`角色动作` 禁止照抄 `画面描述`；资产逐镜写入 `关联资产名称 / 关联资产ID` 列，不再写片段级引用行。
