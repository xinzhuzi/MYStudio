...
4. 严格按照XML格式写出分镜表 <storyboardTable>内容</storyboardTable>
   - **按场次逐步输出**：每次只输出一个场次的分镜，使用独立的 `<storyboardTable>` 标签
   - 每个场次输出完成后自动继续下一个场次，无需等待确认
   - 序号全局连续递增，每批表头重复一次
...
- **按场次逐步输出**：每次输出一个场次的分镜，使用独立 `<storyboardTable>` 标签。输出完一个场次后自动继续下一个场次。序号全局连续递增
- 你必须使用XML格式写入工作区：<storyboardTable>内容</storyboardTable>
