---
name: art_character
description: 人物基础形象生成 · 约束手册
metaData: art_skills
---

# 人物基础形象生成 · 约束手册

---

## 一、基础形象原则

1. **造型即灵魂** — 角色造型是核心锚点，水墨国风造型，工笔线描勾勒
2. **历史画卷人物基准（09-18《三国望神州》DNA 并入）** — 偏写实不走二次元：成年人真实骨骼比例、清晰脸部结构、克制表情、历史人物气质；服装结构可信、甲胄兵器有重量感；禁「二次元美少年模板」也禁纯影视写实——定位=**写实人物+中国画线条审美+游戏角色美术**
3. **线条画出来而非勾边出来** — 线条细、有轻重变化、五官轮廓明确、衣褶清晰但不碎、甲片有结构不过度刻画；禁明显黑色漫画描边、禁粗重欧美插画线
4. **服饰历史锚（题材为历史向时启用；09-18 裁定放宽：汉唐宋明历代均可）** — 汉唐宋明历代服饰考据词均可，按角色设定选定朝代并在同一画面保持统一：汉代冠帽、交领右衽、深衣、宽袖、带具丝带、汉代札甲；唐圆领袍、幞头、半臂、披帛；宋褙子、直领对襟、革带；明曳撒、贴里、圆领袍、乌纱帽等——显著降低「清代装束+仙侠长袍混搭+幻想铠甲+跨朝代元素堆叠」漂移；修仙题材沿用本手册原约束不受此条限制。
5. **底模即基础** — 基础打底服装 + 素颜，后续服化均为叠加层
6. **四视图一致** — 面容/体型/发型/基础服装跨视图高度统一
7. **仙道气质** — 无妆状态仍需体现角色气质（清冷/飘逸/凌厉/温润）

---

## 二、面容约束

> 不再固定五官特征参数，由角色描述（性别/年龄/性格/气质/境界）驱动 AI 自由生成五官，保证人物间外观差异化。

### 通用要求

| 项目 | 约束 |
|---|---|
| 五官 | 由角色描述自然推导，不预设脸型/眼型/眉型/鼻型/唇型 |
| 风格底色 | 水墨国风、工笔白描、写意晕染、浅净平涂底、气韵生动 |
| 气质 | 必须从角色描述提炼整体气质关键词（如清冷出尘/剑意凌厉/温润如玉/妖媚惑人），并写入提示词 |
| 表情 | 中性微表情，符合角色气质与修为境界 |

---

## 三、肤感约束

### 女性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 肤色 | 玉白基调、全身均匀、清透如玉 | 玉白基调、清透如玉、水墨肤色 |
| 光泽 | 工笔淡染、内蕴微光、非哑光 | 工笔淡染、内蕴微光、温润质感 |
| 质感 | 底色平净、色彩淡雅、边缘柔和 | 底色平净、色彩淡雅、边缘如烟 |
| 露肤 | 面部/颈部/手部 | 手部纤细、颈部线条如兰 |

### 男性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 肤色 | 白皙基调、全身均匀、健康清朗 | 白皙基调、健康清朗、水墨肤色 |
| 光泽 | 工笔淡染、内蕴微光 | 工笔淡染、内蕴微光、温润质感 |
| 质感 | 底色平净、干净利落 | 底色平净、工笔淡染、清朗 |

---

## 四、体型约束

### 女性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 身高 | 由角色设定指定，默认范围 160-170cm | {身高}cm tall、{身高描述如：tall elegant woman} |
| 头身比 | 六头身至七头身，水墨古典比例 | 6-7 heads tall proportion、水墨古典比例 |
| 肩颈 | 天鹅颈、肩颈线优美 | 天鹅颈、肩颈优美 |
| 手部 | 纤长白皙、手指自然 | 纤长白皙、自然手指 |
| 体态 | 仙道气质、轻盈飘逸 | 体态轻盈、身姿飘逸 |

### 男性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 身高 | 由角色设定指定，默认范围 175-185cm | {身高}cm tall、{身高描述如：tall imposing man} |
| 头身比 | 六头身至七头身，水墨古典比例 | 6-7 heads tall proportion、水墨古典比例 |
| 肩颈 | 宽阔肩部、颈部有力 | 宽阔肩部、颈部有力 |
| 手部 | 骨节分明、手指自然 | 骨节分明、自然手指 |
| 体态 | 仙风道骨、挺拔端正 | 体态清逸、身姿挺拔 |

---

## 五、基础发型约束

> 仅定义自然发型，发饰在服化衍生环节叠加。

### 女性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 发色 | 默认墨黑；若角色设定或一致性锚点指定其他发色（如银白/霜蓝），以锚点为准，且四视图内保持一致 | {按角色设定/锚点确定的}长发、青丝如瀑 |
| 发长 | 长发及腰 | 长发及腰、长发 |
| 发质 | 工笔线描、发丝清晰 | 工笔线描、清晰发丝、墨色渲染 |
| 造型 | 自然散发、无发饰 | 长发自然散落、无发饰 |

### 男性

| 项目 | 约束 | 提示词 |
|---|---|---|
| 发色 | 默认墨黑；若角色设定或一致性锚点指定其他发色，以锚点为准，且四视图内保持一致 | {按角色设定/锚点确定的}长发、青丝如墨 |
| 发长 | 长发及肩或束发 | 长发及肩、束发 |
| 发质 | 工笔线描、发丝清晰 | 工笔线描、清晰发丝、墨色渲染 |
| 造型 | 自然散发或半束、无发冠 | 长发自然散落、半束长发 |

---

## 六、基础服装约束

> 基础服装无特殊约束，女性为素色道袍长裙，男性为素色道袍长衫。正式法袍在服化衍生环节叠加。

### 女性基础服装

素色道袍长裙，颜色以宣纸白/淡青为主，无花纹装饰。

### 男性基础服装

素色道袍长衫，颜色以宣纸白/淡青为主，无花纹装饰。

### 着装统一规则

- 服装风格统一，确保后续服饰叠加无色彩干扰
- 除面部/手部/颈部外基本覆盖
- 四视图服装款式完全一致
- 基础服装仅为安全打底，焦点在面容与体态

---

## 七、四视图设定图规范

### 视图定义

| 位置 | 视图 | 角度 | 景别 | 要求 | 提示词 |
|---|---|---|---|---|---|
| 左一 | 人像特写 | 正面平视 | 头顶至锁骨 | 从头顶到锁骨完整展示，面部占60%+，五官清晰 | portrait closeup、face detail |
| 左二 | 正视图 | 正面 0° | 全身立像 | 面对镜头、双臂自然、从头顶到脚底完整展示 | front view、full body |
| 右二 | 侧视图 | 右侧 90° | 全身立像 | 纯侧面轮廓清晰、从头顶到脚底完整展示 | side view、profile、full body |
| 右一 | 后视图 | 后方 180° | 全身立像 | 后脑/背部/发尾/脚部清晰、从头顶到脚底完整展示 | back view、rear view、full body |

### 画面规范

| 项目 | 约束 |
|---|---|
| 布局 | 同一画面从左至右并排四视图 |
| 背景 | 宣纸白底色 #F5F0E8 |
| 站姿 | 自然站立、双脚平行微分、双臂自然下垂 |
| 全身展示 | 全身立像必须从头顶到脚底完整入画，严禁裁切 |
| 特写展示 | 人像特写必须从头顶到锁骨完整入画，严禁裁切 |
| 表情 | 中性微表情，符合角色气质 |
| 光线 | 均匀散光，前方主光 + 双侧补光，无硬阴影 |
| 一致性 | 四视图的肤色/体型/发型/面容/基础服装完全一致 |
| 画面比例 | 21:9（运行时支持的最宽画幅；四人物等高并排铺满画面） |

---

## 八、提示词模板（人物轨七段公式 · ma-gongbi-v1）

> 按七段公式以自然语言写出题材正文（段间衔接成文，不是罗列逗号串）。prefix「四、工笔硬锁」是自动层，正文**不抄写、不重复**其内容，也**不出现**「Avoid:/风格底座/TRACK=」等标记；负面一律进 Negative Prompt 段。全文（正文+负面）控制在 300-800 字符。

1. **身份形体**：{性别}修士，{年龄段与境界}，{整体气质：清冷出尘/剑意凌厉/温润如玉…}，{肤色与肤质}，{身高与头身比}，{五官由角色描述自然推导，不预设固定脸型}
2. **动作表情视线**：素颜状态，中性微表情符合气质，目视前方；设定板模式为自然站立、双臂自然下垂；海报/剧情图变体时写明眼神情绪与眉宇唇态（沉静/锐利/含情/冷峻择一），动作到动词精度（手持何物、遮何处、朝向谁），光可叠加侧逆光勾线、发丝留金
3. **服饰材质题材配色**：（女性：素色道袍长裙 / 男性：素色道袍长衫），宣纸白/淡青纯色素色，{材质：素布/云锦薄染}；{发色按角色设定（默认墨黑，一致性锚点优先）}{发长}，工笔线描发丝清晰
4. **人景持物关系**：无持物、无发饰配饰（基础形象安全打底）；纯色背景，无场景
5. **景别机位构图平光**：同一画面左至右并排四视图设定板（人像特写从头顶到锁骨 + 正面/侧面/背面全身立像从头顶到脚底完整入画），21:9，均匀散光
6. **身份不变量**：面容/体型/发型/基础服装跨视图完全一致（四视图一致性）
7. **单一变化轴**：基础形象无变化轴；后续衍生按 L0-L5 逐层单轴变化

**Negative Prompt 基线**：全局五类（错媒介/错光/纸纹脏污/结构错误/文字水印）+ 本轨追加 extra characters, multiple people, cropped head, cropped feet, hair accessories, jewelry, makeup, patterned clothing, complex background, hard shadows, dense wrinkle mesh, bald head, gender-mismatched footwear, tattered clothing, ragged hems, holey beggar rags

---

## 九、约束规则

### 必守

| 编号 | 规则 |
|---|---|
| R1 | 必须为「素颜状态」 |
| R2 | 必须声明基础服装（女性：素色道袍长裙；男性：素色道袍长衫） |
| R3 | 必须声明「无发饰、无配饰」 |
| R4 | 必须指定「宣纸白底色背景」 |
| R5 | 必须指定「四视图一致性」 |
| R6 | 全身立像必须从头顶到脚底完整展示，严禁裁切 |
| R7 | 必须声明角色身高并通过头身比换算约束全身比例（女性默认 160-170cm/6-7头身，男性默认 175-185cm/6-7头身） |
| R8 | 人像特写必须从头顶到锁骨完整展示，严禁裁切头顶 |
| R9 | **衣物完整（全员硬约束）**：衣袍完整可穿、整袖口整下摆、闭合缝线；材质可朴素（素袍灰布），禁止破洞、褴褛、碎边、乞丐装默认造型 |

### 严禁

| 编号 | 严禁 |
|---|---|
| X1 | 基础服装以外的任何服装/配饰/妆容 |
| X2 | 正顶硬光/正底光/冷色光 |
| X3 | 过度美白至无血色 / 肤色发灰 |
| X4 | 复杂场景背景（必须纯色） |
| X5 | 夸张表情/动态姿势 |
| X6 | 全身立像裁切头顶或脚底，必须从头到脚完整入画 |
| X7 | 人像特写裁切头顶，必须从头顶到锁骨完整入画 |
| X8 | 忽略身高和头身比约束，身高必须明确声明并通过头身比换算体现全身比例 |
