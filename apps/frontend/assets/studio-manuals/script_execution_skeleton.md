...
4. 直接输出完整故事骨架（Markdown 格式），依次包含：类型定位、预估时长分配、故事核、隐线、三幕结构、分集决策（每集标注预估时长）、全局删减决策、付费卡点设计。不要使用任何 XML 标签包裹，在本次回复中一次性输出全部内容，不要寒暄。
...
> {判定短剧类型，如 甜宠/战神/逆袭/复仇/重生 等，据此选择节奏框架与情绪基调}

### 预估时长分配

总集数 {N} 集 × 单集时长 {M} 分钟 = 总时长 {N×M} 分钟（集数与单集时长取自【项目配置】，禁止硬编码）；并在分集决策中为每集标注预估时长。
