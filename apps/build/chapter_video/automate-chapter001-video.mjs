...
const buildRoot = resolve(appsRoot, 'build', 'chapter_video');
const generatorScript = resolve(buildRoot, 'build_chapter001_workflow.py');
const continuityPilotScript = resolve(buildRoot, 'generate_chapter001_continuity_sample.py');
...
const reportPath = resolve(appsRoot, 'output', 'automation', 'chapter001-video-report.json');
...
const MAX_CHAPTER_VIDEO_VIDEO_DURATION_SECONDS = 180;
const MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHARS = 4500;
const MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHINESE_CHARS = 2500;
const MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_H2_SECTIONS = 6;
const EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES = 5;
const MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_BULLETS = 50;
const REQUIRED_STORYBOARD_IMAGE_MODE = 'real-ai-reference-image-workflow';
const REQUIRED_CHAPTER_VIDEO_DIRECTOR_PLAN_SECTIONS = [
  '## ① 主题立意与叙事核心',
  '## ② 视觉风格与画面基调',
  '## ③ 叙事结构与节奏规划',
  '## ④ 分场景情绪与画面意图',
...
const REQUIRED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES = ['Sc 1-1', 'Sc 1-2', 'Sc 1-3', 'Sc 1-4', 'Sc 1-5'];
...
function assertImageGenerationNotFrozen(mode, { dryRun = false } = {}) {
  if (dryRun || process.env.MYSTUDIO_IMAGE_GENERATION_FROZEN !== '1') return;
  throw new Error(`MYSTUDIO_IMAGE_GENERATION_FROZEN=1: blocked ChapterVideo image generation mode=${mode}; no provider request was sent`);
}
...
function stopExistingMYStudioInstances() {
  ...
  console.log('[video] closed existing MYStudio instances before ChapterVideo video generation');
}
...
async function runImageProviderProbe() {
  ...
  const report = {
    ...
    command: 'npm run video:chapter001:probe-providers',
  ...
  const probeReportPath = resolve(appsRoot, 'output', 'automation', 'chapter001-provider-probe-report.json');
...
async function runImageGenerationProbe() {
  ...
  const outputPath = process.env.MYSTUDIO_IMAGE_PROBE_OUTPUT_PATH
    || resolve(appsRoot, 'output', 'automation', 'chapter001-generation-probe', 'dugu-turnaround-probe.png');
  const generationProbeReportPath = process.env.MYSTUDIO_IMAGE_PROBE_REPORT_PATH
    || resolve(appsRoot, 'output', 'automation', 'chapter001-generation-probe-report.json');
  ...
  if (result.status !== 0) {
    ...
    const failureReport = {
      ok: false,
      generatedAt: new Date().toISOString(),
      command: 'npm run video:chapter001:probe-generation',
...
function runCandidateColorAudit(outputPath) {
  const result = run('python3', [
    ...
    [
      ...
      'from apps.build.chapter_video.pipeline.gongbi_v2 import write_color_audit',
...
async function runAuthorizedContinuityAssetCandidate() {
  ...
  if (result.status !== 0) {
    ...
    writeDurableJsonReport(candidateReportPath, {
      ok: false,
      generatedAt: new Date().toISOString(),
      command: 'video:chapter001 --continuity-asset-candidate',
...
async function runContinuityPilot(fullChapter = false) {
  ...
  const outputDir = (fullChapter
    ...
    || resolve(
      ...
      'output',
      'automation',
      `chapter001-continuity-${fullChapter ? 'full' : 'pilot'}-${timestamp}`,
    );
  const pilotArgs = [
  ...
  const result = run('python3', pilotArgs, {
    env: {
      MYSTUDIO_CHAPTER_VIDEO_STORYBOARD_IMAGE_MODE: REQUIRED_STORYBOARD_IMAGE_MODE,
...
function requireStoryboardPromptIntegrity(generated) {
  ...
  const requiredCounts = [
    ...
    ['storyboardPromptsWithStyleLock', '分镜提示词风格锁不完整'],
...
function requireDirectorPlanIntegrity(generated) {
  if (Number(generated.directorPlanChars) < MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHARS) {
    throw new Error(`导演规划正文过短: ${generated.directorPlanChars ?? 'missing'}/${MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHARS}`);
  }
  if (Number(generated.directorPlanChineseChars) < MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHINESE_CHARS) {
    throw new Error(`导演规划中文有效字数过少: ${generated.directorPlanChineseChars ?? 'missing'}/${MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_CHINESE_CHARS}`);
  }
  if (Number(generated.directorPlanH2Sections) < MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_H2_SECTIONS) {
    throw new Error(`导演规划二级章节不足: ${generated.directorPlanH2Sections ?? 'missing'}/${MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_H2_SECTIONS}`);
  }
  if (Number(generated.directorPlanSceneSections) !== EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES) {
    throw new Error(`导演规划 Sc 场景段必须为 ${EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES}: ${generated.directorPlanSceneSections ?? 'missing'}`);
  }
  if (Number(generated.directorPlanBulletCount) < MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_BULLETS) {
    throw new Error(`导演规划 bullet 细项不足: ${generated.directorPlanBulletCount ?? 'missing'}/${MIN_CHAPTER_VIDEO_DIRECTOR_PLAN_BULLETS}`);
  }

  const requiredSections = generated.directorPlanRequiredSectionsPresent || {};
  const missingSections = REQUIRED_CHAPTER_VIDEO_DIRECTOR_PLAN_SECTIONS.filter((section) => requiredSections[section] !== true);
  ...
  const missingScenes = REQUIRED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES.filter((sceneId) => requiredScenes[sceneId] !== true);
  if (missingScenes.length > 0) {
    throw new Error(`导演规划缺少必需 Sc 场景段: ${missingScenes.join(', ')}`);
  }

  if (Number(generated.directorPlanStructuredSceneIntents) !== EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES) {
    throw new Error(
      `结构化导演规划场景意图数量异常: ${generated.directorPlanStructuredSceneIntents ?? 'missing'}/${EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES}`,
    );
  }
  if (Number(generated.directorPlanStructuredSceneIntentsComplete) !== EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES) {
    throw new Error(
      `结构化导演规划场景意图不完整: ${generated.directorPlanStructuredSceneIntentsComplete ?? 'missing'}/${EXPECTED_CHAPTER_VIDEO_DIRECTOR_PLAN_SCENES}`,
    );
  }
...
function writeFailureReport(generated, error, timelineResult = null) {
  ...
  const report = {
    ok: false,
    generatedAt: new Date().toISOString(),
    command: 'npm run video:chapter001',
    generatorScript,
    ...
    storyboardPromptsWithStyleLock: generated?.storyboardPromptsWithStyleLock,
...
if (probeGenerationOnly) {
  ...
  assertImageGenerationNotFrozen('probe-generation');
  stopExistingMYStudioInstances();
...
if (continuityAssetCandidateOnly) {
  ...
  assertImageGenerationNotFrozen('continuity-asset-candidate', {
...
if (continuityPilotOnly) {
  ...
  assertImageGenerationNotFrozen('continuity-pilot', {
...
if (continuityFullChapterOnly) {
  ...
  assertImageGenerationNotFrozen('continuity-full-chapter', {
    dryRun: process.env.MYSTUDIO_CONTINUITY_PILOT_DRY_RUN === '1',
  });
  stopExistingMYStudioInstances();
...
assertImageGenerationNotFrozen('chapter001-video');
const visualContinuityPreflight = requireVisualContinuityPreflight();
console.log(
...
try {
  ...
  generated = parseGeneratorOutput(run('python3', [generatorScript, '--run'], {
    env: {
      ...
      MYSTUDIO_CHAPTER_VIDEO_ALLOW_STORYBOARD_BOOTSTRAP: '0',
      MYSTUDIO_CHAPTER_VIDEO_STORYBOARD_IMAGE_MODE: REQUIRED_STORYBOARD_IMAGE_MODE,
      MYSTUDIO_CHAPTER_VIDEO_USE_APPROVED_STORYBOARDS: '1',
      ...
      MYSTUDIO_CHAPTER_VIDEO_REMOTION_ONLY: '1',
...
if (generated.ttsMode.includes('silent-visual-preview') && process.env.MYSTUDIO_CHAPTER_VIDEO_SILENT_PREVIEW !== '1') {
  throw new Error('不能使用静音视觉预览作为最终音频；如只做画面检查，显式设置 MYSTUDIO_CHAPTER_VIDEO_SILENT_PREVIEW=1');
}
...
if (finalVideoDuration > MAX_CHAPTER_VIDEO_VIDEO_DURATION_SECONDS) {
  throw new Error(`最终视频时长超过3分钟规格: ${finalVideoDuration.toFixed(2)}s/${MAX_CHAPTER_VIDEO_VIDEO_DURATION_SECONDS}s`);
}
