...
The runner reuses the production ChapterVideo prompt/reference builders, writes only
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "apps/output/automation/chapter001-continuity-sample"
DEFAULT_PAID_REQUEST_LEDGER = REPO_ROOT / "apps/output/automation/chapter001-paid-image-request-ledger.jsonl"
...
def dry_run_provider_config(generator) -> dict[str, Any]:
    manifest = gongbi_v2.load_reference_capability_manifest()
    verified = [
    ...
    return generator.with_gongbi_v2_contract({
...
def parse_review_checks(values: list[str]) -> dict[str, bool]:
    ...
    for value in values:
        ...
        if not separator or field not in gongbi_v2.HUMAN_REVIEW_CHECKLIST_FIELDS:
...
def summarize_reference_capabilities(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        reason = ""
        try:
            gongbi_v2.assert_reference_capability(capability, roles)
        ...
        bindings.setdefault(binding, {
            ...
            "semanticRoleEvidence": gongbi_v2.reference_semantic_role_evidence(capability),
    ...
    return {
        "schemaVersion": gongbi_v2.REFERENCE_CAPABILITY_SCHEMA_VERSION,
...
def summarize_reference_visual_audits(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        for reason in audit.get("failedGates") or ["missing_reference_visual_audit"]:
            reason_counts[reason] = reason_counts.get(reason, 0) + 1
    return {
        "schemaVersion": "chapter_video-reference-visual-audit-summary-v1",
...
def valid_human_review(
    ...
    requires_v2_checklist = entry.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS
    valid = bool(
        ...
        and (
            ...
            or gongbi_v2.is_complete_approved_review_checklist(record.get("reviewChecklist"))
        )
    )
...
def previous_approved_frame_manifest(
    ...
    if previous_entry.get("styleContractVersion") not in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS:
...
def record_human_review(
    ...
    requires_v2_checklist = entry.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS
    normalized_checklist = gongbi_v2.normalize_review_checklist(review_checklist)
    if (
        review_status == "approved"
        and requires_v2_checklist
        and not gongbi_v2.is_complete_approved_review_checklist(normalized_checklist)
    ):
...
def main() -> None:
    ...
    for index in target_indexes:
        ...
        if not args.dry_run:
            ...
            logical_job = "chapter001-continuity-full" if args.full_chapter else "chapter001-continuity-pilot"
            ...
            try:
                ...
                color_audit = generator.audit_gongbi_v2_output(output_path)
            ...
            color_audit = {
                "version": gongbi_v2.COLOR_AUDIT_VERSION,
                "status": "not-run",
