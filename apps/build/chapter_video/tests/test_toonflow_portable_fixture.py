...
from apps.build.chapter_video.pipeline.build_toonflow_portable_fixture import (
    build_fixture,
...
class ToonflowPortableFixtureTest(unittest.TestCase):
    ...
    def test_verify_fixture_rejects_paths_that_escape_fixture_root(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            manifest_path = self._build_fixture(root)
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["storyboardRows"][0]["goldenImage"]["relativePath"] = "../../outside.png"
            manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

            with self.assertRaisesRegex(RuntimeError, "golden image path escapes fixture root"):
                verify_fixture(manifest_path)
