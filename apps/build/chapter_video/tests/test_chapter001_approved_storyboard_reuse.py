...
def load_generator():
    path = Path(__file__).resolve().parents[1] / "build_chapter001_workflow.py"
...
class Chapter001ApprovedStoryboardReuseTest(unittest.TestCase):
    ...
    def test_resolves_only_current_human_approved_project_image(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(resolved["styleContractVersion"], "gongbi-v2")
    ...
    def test_main_storyboard_entrypoint_uses_primary_assets_with_existing_provider(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            approved_input = {
                ...
                "styleContractVersion": module.gongbi_v2.STYLE_CONTRACT_VERSION,
            }
            config = {
                "providerName": "mikoto",
                "model": "gpt-image-2",
                ...
                "resolution": "1K",
                "styleContractVersion": module.gongbi_v2.STYLE_CONTRACT_VERSION,
                "styleContractFingerprint": module.gongbi_v2.style_contract_fingerprint(),
                "promptAuditVersion": module.gongbi_v2.PROMPT_AUDIT_VERSION,
            }

            with patch.object(module, "audit_gongbi_v2_output", return_value={"status": "pass"}):
                result = module.generate_storyboard_frame_with_references(
                    root / "frame.png",
                    {
                        "id": "sb-chapter-001-001",
