...
def expected_report_shots(selected_shot: int | None) -> list[int]:
    ...
    if selected_shot not in EXPECTED_SHOTS:
        raise RuntimeError(f"--shot 必须是 {EPISODE_ID} 的 1-43 镜")
...
def validate_report(
    ...
    if selected_shot is not None:
        expected_storyboard_id = f"sb-{EPISODE_ID}-{selected_shot:03d}"
...
def promoted_media_ref(update: dict[str, Any]) -> tuple[dict[str, Any], str, str]:
    flow_id = f"storyboard-flow-{EPISODE_ID}-{int(update['index']):03d}"
...
def build_promotion_plan(
    ...
    for entry in entries:
        ...
        destination = (
            project
            / f"workflow-images/storyboards/{EPISODE_ID}/approved-revisions"
...
def preflight_update_payloads(
    ...
    approved_root = (
        project / f"workflow-images/storyboards/{EPISODE_ID}/approved-revisions"
    ).resolve()
