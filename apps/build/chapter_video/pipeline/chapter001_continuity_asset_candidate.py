"""Validate paid-safe ChapterVideo chapter-001 continuity asset manifests."""
...
SCHEMA_VERSION = "chapter_video-continuity-asset-candidate-v2"
...
def validate_manifest(
    ...
    *,
    ...
    expected_capability = gongbi_v2.resolve_reference_capability(
        provider_name,
        model,
        reference_roles,
    )
    ...
    gongbi_v2.assert_reference_capability(expected_capability, reference_roles)
    ...
    expected_prompt_audit = {
        **gongbi_v2.prompt_quality_audit(prompt, reference_roles, expected_capability),
        "promptSha256": prompt_sha256,
    }
    if manifest.get("promptAudit") != expected_prompt_audit:
        raise RuntimeError("连续性资产候选 promptAudit 与当前提示词或 V2 规则不匹配")
    gongbi_v2.assert_prompt_quality_audit(expected_prompt_audit)
    expected_prompt_contract_revision = gongbi_v2.REFERENCE_REPLACEMENT_PROMPT_VERSION
    ...
    expected_replacement_audit = gongbi_v2.reference_replacement_prompt_audit(
        prompt,
        asset_kind,
    )
    ...
    gongbi_v2.assert_reference_replacement_prompt_audit(expected_replacement_audit)
