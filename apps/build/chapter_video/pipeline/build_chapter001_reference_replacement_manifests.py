...
from apps.build.chapter_video.pipeline import chapter001_continuity_asset_candidate as candidate_contract
...
SOURCE_PLAN = TASK_DIR / "research/gongbi-v2-shot001-reference-replacement-plan-20260722-r01.json"
OUTPUT_PLAN = TASK_DIR / "research/gongbi-v2-shot001-reference-replacement-plan-20260723-r03.json"
MANIFEST_DIR = TASK_DIR / "research/gongbi-v2-shot001-reference-replacement-manifests-20260723-r02"
OUTPUT_ROOT = REPO_ROOT / "apps/output/automation/chapter001-v2-bible-replacements-20260723-r03"
PROMPT_CONTRACT_REVISION = gongbi_v2.REFERENCE_REPLACEMENT_PROMPT_VERSION
...
def _section(prompt: str, name: str) -> str:
    value = gongbi_v2.extract_prompt_section(prompt, name)
    if not value:
...
def _safe_asset_text(value: str) -> str:
    rendered = gongbi_v2.render_prompt_safe_negative_constraints(value)
    return gongbi_v2.render_prompt_safe_story_facts(rendered)
...
def _compact_medium_lock(asset_kind: str) -> str:
    if asset_kind == "scene":
        return (
            "gongbi-v3 2D彩色工笔水墨；媒介规则优先于参考图中的数字渲染。"
...
def build_review_plan(source_plan_path: Path = SOURCE_PLAN) -> dict[str, Any]:
    ...
    if source_plan.get("schemaVersion") != "chapter_video-v2-reference-replacement-plan-v1":
    ...
    for source_job in source_plan.get("jobs") or []:
        ...
        capability = gongbi_v2.resolve_reference_capability("mikoto", "gpt-image-2", roles)
        audit = {
            **gongbi_v2.prompt_quality_audit(prompt, roles, capability),
            "promptSha256": candidate_contract.sha256_text(prompt),
        }
        gongbi_v2.assert_prompt_quality_audit(audit)
        replacement_audit = gongbi_v2.reference_replacement_prompt_audit(
            prompt,
            str(job["assetKind"]),
        )
        gongbi_v2.assert_reference_replacement_prompt_audit(replacement_audit)
    ...
    return {
        "schemaVersion": "chapter_video-v2-reference-replacement-plan-v3",
        "promptContractRevision": PROMPT_CONTRACT_REVISION,
...
def build_candidate_manifest(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "logicalJob": "chapter001-continuity-reference-replacement",
        ...
        "styleContract": {
            ...
            "fingerprint": gongbi_v2.style_contract_fingerprint(),
            "sourceProvenance": gongbi_v2.SOURCE_PROVENANCE,
        },
        "prompt": job["prompt"],
