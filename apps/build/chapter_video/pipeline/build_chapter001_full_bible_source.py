...
try:
    ...
    from apps.build.chapter_video.path_resolver import resolve_asset_files_dir, resolve_project_dir
except ModuleNotFoundError:
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from gongbi_v2 import gongbi_v2
...
ART_DIRECTION_VERSION = gongbi_v2.STYLE_CONTRACT_VERSION
EXPECTED_VERSION_COUNT = 32
SOURCE_MATRIX_STEM = "chapter001-gongbi-v2-32-version-source-matrix"
...
def generation_jobs() -> list[dict[str, Any]]:
    style = (
        f"{gongbi_v2.DERIVED_ASSET_STYLE_LOCK}"
...
def write_plan() -> dict[str, Any]:
    ...
    prompts_path = write_versioned_research_artifact(
        "chapter001-gongbi-v2-generation-prompts", 1,
...
def continuity_helpers():
    ...
    path = REPOSITORY_ROOT / "apps/build/chapter_video/build_chapter001_workflow.py"
