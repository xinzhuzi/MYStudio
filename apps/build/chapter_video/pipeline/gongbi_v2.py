"""Local ChapterVideo gongbi-v2 prompt, capability, and review contracts.
...
STYLE_CONTRACT_VERSION = "gongbi-v3"
# 存量台账/已批准资产为 gongbi-v2;消费侧(审批/回放/参考继承)对两版本均接受。
ACCEPTED_STYLE_CONTRACT_VERSIONS = frozenset({"gongbi-v2", STYLE_CONTRACT_VERSION})
PROMPT_AUDIT_VERSION = "gongbi-v3-prompt-audit-v1"
COLOR_AUDIT_VERSION = "gongbi-v2-color-audit-v3"
REFERENCE_CAPABILITY_SCHEMA_VERSION = "chapter_video-reference-capability-v1"
SOURCE_PROVENANCE = (
    "MA/.claude/skills/ma-imagegen/knowledge/art-direction/"
    "mystudio-chapter_video-gongbi-style-contract.md"
)
COLOR_AUDIT_SOURCE_PROVENANCE = (
    "MA/.claude/skills/ma-imagegen/scripts/"
    "gongbi/chapter_video_gongbi_restyle.py:audit_polychrome_image"
)
REFERENCE_CAPABILITY_MANIFEST = Path(__file__).with_name(
    "gongbi_v2_reference_capabilities.json"
)
...
STORYBOARD_MEDIUM_LOCK = (
    "gongbi-v3：2D彩色工笔水墨手绘剧情关键帧；媒介规则优先于参考图中的数字渲染、"
...
DERIVED_ASSET_STYLE_LOCK = (
    "gongbi-v3：彩色工笔资产：先以连续白描和铁线描锁定脸、手、发丝、"
...
REFERENCE_COLOR_AUDIT_VERSION = "gongbi-v2-reference-color-audit-v2"
...
REFERENCE_REPLACEMENT_PROMPT_VERSION = "gongbi-v3-reference-replacement-v1"
...
INCOMPATIBLE_WARDROBE_VERSIONS = {
    "dock-ragged": (
        "dock-ragged 与 gongbi-v3 完整衣物合同不兼容；"
...
def assert_prompt_quality_audit(audit: dict[str, Any]) -> None:
    if audit.get("status") != "pass":
        raise RuntimeError("工笔 V2 提示词审计失败: " + ", ".join(audit.get("violations") or ["unknown"]))
