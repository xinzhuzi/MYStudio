...
BIBLE_VERSION = "v5"
ART_DIRECTION_VERSION = "gongbi-v2"
PILOT_PROPS = {
...
DOCK_BIBLE = {
    ...
    "lightingDesign": "阴天晨雾中的柔和均匀平光与浅净哑光平涂底，湿石与河面保留轻薄有彩色层次的反光，人物脸手清晰可读。",
...
def load_continuity_helpers():
    ...
    generator_path = Path(__file__).resolve().parents[1] / "build_chapter001_workflow.py"
...
def prepare_bibles(
    ...
    bible_root = project_dir / f"continuity-bibles/{EPISODE_ID}" / bible_version
    ...
    backup_dir = project_dir / f"backups/{EPISODE_ID}-continuity-bible" / timestamp
    ...
    dock["viewpoints"] = [{
        ...
        "shotIds": [f"sb-{EPISODE_ID}-{index:03d}" for index in range(1, 13)],
