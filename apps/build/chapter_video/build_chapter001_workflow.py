...
try:
    from apps.build.chapter_video.pipeline import gongbi_v2
    from apps.build.chapter_video.path_resolver import (
        env_path,
    ...
    from pipeline import gongbi_v2
...
FINAL_NAME = (
    f"{WORK_TITLE}_EP01_{EPISODE_TITLE}.mp4"
    if EPISODE_TITLE
    else f"{WORK_TITLE}_EP01.mp4"
)
EPISODE_DISPLAY_TITLE = (
    f"{WORK_TITLE} EP01：{EPISODE_TITLE}" if EPISODE_TITLE else f"{WORK_TITLE} EP01"
)
...
def _load_local_story_data():
    """章一故事数据(未发表小说改编)不入公开仓,从本地档案读取。

    数据含 VOICE_REFERENCE_TEXT_FALLBACK / CHAPTER_001_DIRECTOR_SCENES /
    CHAPTER_001_SHOTS;档案结构见 ~/.zcode/mystudio-local/chapter001-story-data.json。
    """
    story_data_path = Path(os.environ.get(
        "MYSTUDIO_CHAPTER001_STORY_DATA",
        str(Path.home() / ".zcode" / "mystudio-local" / "chapter001-story-data.json"),
    ))
    return json.loads(story_data_path.read_text(encoding="utf-8"))


_STORY_DATA = _load_local_story_data()
VOICE_REFERENCE_TEXT_FALLBACK = _STORY_DATA["voice_reference_text_fallback"]
...
TTS_TOKEN = os.environ.get("MANYING_TTS_CONTROL_TOKEN") or f"chapter_video-{uuid.uuid4().hex}"
...
SILENT_PREVIEW = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_SILENT_PREVIEW") == "1"
REUSE_AUDIO_DIR = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_REUSE_AUDIO_DIR")
REPO_ROOT = Path(__file__).resolve().parents[3]
BACKEND_ROOT = REPO_ROOT / "apps" / "backend"
NODE_STORYBOARD_IMAGE_HELPER = REPO_ROOT / "apps" / "build" / "chapter_video" / "generate-storyboard-image.mjs"
SKIP_PROJECT_WRITE = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_SKIP_PROJECT_WRITE") == "1"
SKIP_SCENE_EXPORTS = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_SKIP_SCENE_EXPORTS") == "1"
REMOTION_ONLY = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_REMOTION_ONLY") == "1"
ALLOW_STORYBOARD_BOOTSTRAP = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_ALLOW_STORYBOARD_BOOTSTRAP") == "1"
USE_APPROVED_STORYBOARD_IMAGES = os.environ.get("MYSTUDIO_CHAPTER_VIDEO_USE_APPROVED_STORYBOARDS") == "1"
...
STORYBOARD_IMAGE_GENERATION_MODE = (
    os.environ.get("MYSTUDIO_CHAPTER_VIDEO_STORYBOARD_IMAGE_MODE", ASSET_COMPOSITE_IMAGE_MODE).strip()
...
MODEL_REFERENCE_PREFLIGHT_SCHEMA_VERSION = "chapter_video-model-reference-preflight-v1"
REFERENCE_TRANSPORT_SCHEMA_VERSION = "chapter_video-reference-transport-v1"
...
def with_gongbi_v2_contract(config):
    """Attach immutable prompt-contract metadata to every real storyboard request."""
    if not config:
        return config
    return {
        **config,
        "styleContractVersion": gongbi_v2.STYLE_CONTRACT_VERSION,
        "styleContractFingerprint": gongbi_v2.style_contract_fingerprint(),
        "promptAuditVersion": gongbi_v2.PROMPT_AUDIT_VERSION,
    }
...
def storyboard_image_provider_config():
    ...
    if provider_configs:
        ...
        return with_gongbi_v2_contract({
            **first,
    ...
    return with_gongbi_v2_contract(config)
...
CHAPTER_VIDEO_STORYBOARD_STYLE_PROMPT = gongbi_v2.STORYBOARD_STYLE_LOCK
CHAPTER_VIDEO_STORYBOARD_MEDIUM_PROMPT = gongbi_v2.STORYBOARD_MEDIUM_LOCK
CHAPTER_VIDEO_STORYBOARD_COLOR_MATERIAL_PROMPT = gongbi_v2.STORYBOARD_COLOR_MATERIAL_LOCK
CHAPTER_VIDEO_STORYBOARD_LIGHT_PROMPT = gongbi_v2.STORYBOARD_LIGHT_LOCK
CHAPTER_VIDEO_DERIVED_ASSET_STYLE_PROMPT = gongbi_v2.DERIVED_ASSET_STYLE_LOCK
CHAPTER_VIDEO_STORYBOARD_NEGATIVE_CONSTRAINTS = gongbi_v2.STORYBOARD_NEGATIVE_CONSTRAINTS
CHAPTER_VIDEO_STORYBOARD_FRAME_NEGATIVE_CONSTRAINTS = gongbi_v2.STORYBOARD_FRAME_NEGATIVE_CONSTRAINTS
...
CHAPTER_VIDEO_REFERENCE_BINDING_ALIASES = {
    "独孤剑尘": ["独孤剑尘", "独孤", "灰衫客"],
    "监工赵四": ["监工赵四", "赵四"],
    "赵四": ["赵四", "监工赵四"],
    "小杂役": ["小杂役"],
...
CHAPTER_VIDEO_VISIBLE_ROLE_ALIASES = {
    key: value
    for key, value in CHAPTER_VIDEO_REFERENCE_BINDING_ALIASES.items()
    if key in {
        "独孤剑尘",
        "监工赵四",
        "赵四",
...
def storyboard_reference_aliases(reference, index):
    ...
    for alias_name, candidates in CHAPTER_VIDEO_REFERENCE_BINDING_ALIASES.items():
...
def audit_storyboard_reference_alias_ownership(reference_images):
    ...
    return {
        "schemaVersion": "chapter_video-reference-alias-ownership-v1",
...
def build_storyboard_reference_continuity_rules(reference_images):
    ...
    for index, reference in enumerate(reference_images or [], 1):
        ...
        if label == "角色":
            character_bucket = (
                "角色"
                if gongbi_v2.is_v2_wardrobe_compatible(reference.get("wardrobeVersion"))
                else "角色仅身份"
            )
...
def build_storyboard_bible_rules(reference_images, *, enforce_v2_reference_compatibility=True):
    ...
    for index, reference in enumerate(reference_images or [], 1):
        ...
        if label == "角色":
            ...
            anchor_text = gongbi_v2.render_prompt_safe_story_facts(
            ...
            avoid_text = gongbi_v2.render_prompt_safe_negative_constraints(avoid_text)
            source_wardrobe_version = gongbi_v2.render_prompt_wardrobe_version(
                str(reference.get("wardrobeVersion") or "")
            )
            if enforce_v2_reference_compatibility:
                gongbi_v2.assert_v2_wardrobe_compatible(source_wardrobe_version)
            wardrobe_version = gongbi_v2.render_prompt_safe_wardrobe_version(
...
def storyboard_light_prompt(storyboard):
    ...
    scene_light = gongbi_v2.storyboard_scene_lighting(view_id)
    if scene_light:
        return scene_light
    ...
    if normalized["personFree"]:
        ...
        for character in normalized["visibleCharacters"]:
            ...
            parts.append(
                ...
                f"从{gongbi_v2.render_prompt_safe_story_facts(character['actionIn'])}"
                f"到{gongbi_v2.render_prompt_safe_story_facts(character['actionOut'])}"
            )
    parts.append(
        f"整镜从{gongbi_v2.render_prompt_safe_story_facts(normalized['actionIn'])}"
        f"承接到{gongbi_v2.render_prompt_safe_story_facts(normalized['actionOut'])}"
    )
    return "；".join(parts)
...
def build_storyboard_reference_visual_audit(reference_images, expected_time_of_day=""):
    ...
    for order, reference in enumerate(reference_images or [], 1):
        ...
        try:
            gongbi_v2.assert_v2_wardrobe_compatible(wardrobe_version)
        ...
        if not source or source.startswith(("http://", "https://", "data:image/")):
            ...
            try:
                ...
                color_audit = gongbi_v2.audit_reference_color(
                    source_path,
    ...
    return {
        "schemaVersion": "chapter_video-reference-visual-audit-v1",
        "styleContractVersion": gongbi_v2.STYLE_CONTRACT_VERSION,
        "expectedTimeOfDay": expected_time_of_day or None,
...
def assert_storyboard_reference_visual_audit(audit):
    if audit.get("status") == "pass":
        return
    raise RuntimeError(
        "工笔 V2 参考图预检失败: " + "、".join(audit.get("failedGates") or ["unknown"])
    )
...
def find_visible_role_mentions(visual_prompt):
    visible = []
    for role_name, aliases in CHAPTER_VIDEO_VISIBLE_ROLE_ALIASES.items():
...
def build_storyboard_prompt_audit(storyboard, final_prompt, reference_images, raw_visual_prompt):
    ...
    missing_visible_role_refs = [
        ...
        if not any(alias in role_reference_aliases for alias in CHAPTER_VIDEO_VISIBLE_ROLE_ALIASES.get(role_name, [role_name]))
    ]
    ...
    v2_audit = gongbi_v2.prompt_quality_audit(
        final_prompt,
        reference_roles,
    ...
    return {
        ...
        "hasStyleLock": (
            gongbi_v2.STYLE_CONTRACT_VERSION in final_prompt
...
def summarize_storyboard_prompt_manifest(manifest):
    return {
        ...
        "storyboardPromptsWithStyleLock": sum(1 for item in manifest if item.get("hasStyleLock")),
...
def build_storyboard_image_prompt(storyboard, reference_images, *, enforce_v2_reference_compatibility=True):
    if enforce_v2_reference_compatibility:
        for reference in reference_images or []:
            if storyboard_reference_type_label(reference) == "角色":
                gongbi_v2.assert_v2_wardrobe_compatible(reference.get("wardrobeVersion"))
    visual_prompt = gongbi_v2.render_prompt_safe_story_facts(
    ...
    parts.append("【可变化项】只变化当前分镜需要的景别、动作、表情、局部光影、雾气和前景遮挡；不要改变已限定继承的角色身份、场景空间、道具核心特征或作品主风格。")
    parts.append(f"【媒介层级】{CHAPTER_VIDEO_STORYBOARD_MEDIUM_PROMPT}。")
    parts.append("【构图空间】16:9横版国风漫剧剧情关键帧，前中远景层次清楚，主体动作、角色位置和道具关系可读，不要画成资产设定页。")
    parts.append(f"【色彩材质】{CHAPTER_VIDEO_STORYBOARD_COLOR_MATERIAL_PROMPT}。")
    light_prompt = gongbi_v2.render_prompt_safe_story_facts(storyboard_light_prompt(storyboard))
    parts.append(f"【光影】{light_prompt}。{CHAPTER_VIDEO_STORYBOARD_LIGHT_PROMPT}。")
    parts.append(f"【反向约束】{CHAPTER_VIDEO_STORYBOARD_FRAME_NEGATIVE_CONSTRAINTS}。")
...
def build_derived_asset_image_prompt(parent_name, state_name, reason, asset_type):
    ...
    safe_state_name = gongbi_v2.render_prompt_safe_story_facts(state_name)
    safe_reason = gongbi_v2.render_prompt_safe_story_facts(reason)
    if asset_type == "character":
        parts = [
            ...
            f"【风格】{CHAPTER_VIDEO_DERIVED_ASSET_STYLE_PROMPT}。",
            (
                "【参考继承边界】@图1仅继承同一角色身份、面容、体态、发型和比例结构；"
                "V2媒介、综合色彩、完整衣物及当前衍生状态优先，不复制参考图的破损衣物、脏灰媒介、原构图或旧光影。"
            ),
            f"【反向约束】{CHAPTER_VIDEO_STORYBOARD_NEGATIVE_CONSTRAINTS}，face drift，identity changed，different person，pose changed，added scene，added handheld prop，inconsistent costume between views，cropped body。",
        ]
        return " ".join(part for part in parts if part).strip()
    ...
    parts = [
        ...
        f"【反向约束】{CHAPTER_VIDEO_STORYBOARD_NEGATIVE_CONSTRAINTS}。",
    ]
    return " ".join(part for part in parts if part).strip()
...
def parse_storyboard_image_reuse_after_timestamp(value):
    ...
    try:
        ...
        raise RuntimeError(f"MYSTUDIO_CHAPTER_VIDEO_REUSE_STORYBOARD_IMAGES_AFTER 不是有效 ISO 时间: {raw}") from error


def can_reuse_storyboard_image(image_path):
    if os.environ.get("MYSTUDIO_CHAPTER_VIDEO_REUSE_STORYBOARD_IMAGES") != "1":
        return False
    threshold = parse_storyboard_image_reuse_after_timestamp(
        os.environ.get("MYSTUDIO_CHAPTER_VIDEO_REUSE_STORYBOARD_IMAGES_AFTER", "")
    )
...
def request_storyboard_image_generation(prompt, reference_images, config):
    if config.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS:
        ...
        gongbi_v2.assert_reference_capability(
...
def storyboard_reference_capability(config, reference_images):
    ...
    return gongbi_v2.resolve_reference_capability(
        provider_name,
...
def style_reference_sha256_from_references(reference_images):
    hashes = [
        str(value).lower()
        for reference in reference_images or []
        if reference.get("referenceRole") == gongbi_v2.STYLE_REFERENCE_ROLE
        for value in reference.get("referenceImageSha256") or []
        if str(value).strip()
    ]
...
def with_storyboard_v2_reference_contract(config, reference_images):
    ...
    if request_config.get("styleContractVersion") not in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS:
    ...
    if capability.get("status") == "verified" and capability_request_mode != request_mode:
        ...
        capability["fingerprint"] = gongbi_v2.reference_capability_fingerprint(capability)
    ...
    request_config["styleContractFingerprint"] = gongbi_v2.style_contract_fingerprint(
        style_reference_sha256 or None
    )
    return request_config


def audit_gongbi_v2_output(image_path):
    report = gongbi_v2.write_color_audit(Path(image_path))
    if report["status"] != "pass":
        raise RuntimeError("工笔 V2 色彩审计失败: " + ", ".join(report["failedGates"]))
    return report
...
def create_storyboard_image_workflow_graph(storyboard, prompt, result_image_path, reference_images, config, created_at):
    ...
    return {
        "id": flow_id,
        "name": f"{WORK_TITLE} · 分镜 {storyboard['index']} 图片工作流",
...
def generate_storyboard_frame_with_references(
    ...
    reference_visual_audit = (
        ...
        if request_config.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS
        else {
            "schemaVersion": "chapter_video-reference-visual-audit-v1",
            "status": "not-run",
    ...
    if (
        ...
        and request_config.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS
        and (not approved_storyboard_image or approved_storyboard_image.get("styleContractVersion") not in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS)
    ):
        raise RuntimeError("gongbi-v2 不得复用缺少 V2 契约证据的旧分镜图")
    ...
    if not reused_existing_image:
        ...
        prepared_reference_images, reference_preflight = prepare_storyboard_model_reference_images(
            references,
            denoise=request_config.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS,
        )
    ...
    color_audit = None
    if request_config.get("styleContractVersion") in gongbi_v2.ACCEPTED_STYLE_CONTRACT_VERSIONS:
        color_audit = audit_gongbi_v2_output(result_file)
...
def image_source_to_path(source):
    ...
    path = Path(source)
    # Projects relocated to an external location keep legacy absolute paths
    # under projects/_p/<projectId>/ in stored workflow data; remap them to
    # the currently resolved project directory when the legacy file is gone.
    if not path.is_file():
        parts = path.parts
        for index, part in enumerate(parts):
            if part == "_p" and index + 1 < len(parts):
                if str(path).startswith(str(Path(*parts[: index + 2]))):
                    remapped = PROJECT.joinpath(*parts[index + 2 :])
                    if remapped.is_file():
                        return remapped
                break
    return path
...
def save_generated_image_url(image_url, output_path):
    ...
    if image_url.startswith("data:image/"):
        ...
        return
    request = urllib.request.Request(image_url, headers={"User-Agent": "MYStudio/ChapterVideoStoryboardImage"})
...
CHAPTER_001_DIRECTOR_SCENES = _STORY_DATA["director_scenes"]
...
# 逐镜头(场景/画面/角色/台词/音效/资产/时长)来自本地档案;元素结构与原内嵌元组一致
CHAPTER_001_SHOTS = [tuple(shot) for shot in _STORY_DATA["shots"]]
...
def load_asset_image_rows():
    if not ASSET_DB.exists():
        return []
    with closing(sqlite3.connect(str(ASSET_DB))) as conn:
        with conn:
    ...
    for preferred_name in preferences:
        for row in audio_rows:
            if row.get("name") == preferred_name:
                return {
                    "profileId": f"chapter_video-{asset_key(speaker)}-voice-reference",
                    "speaker": speaker,
...
def resolve_fixed_voice_bindings(
    ...
    for speaker_id, descriptor in sorted(speaker_descriptors.items()):
        ...
        profile_id = f"chapter_video-{asset_key(speaker_id)}-voice-reference"
...
def managed_python_executable_path(runtime_dir, platform=None):
...
def ensure_tts_profile(profile):
    return http_json("POST", "/profiles", {
        ...
        "name": f"{WORK_TITLE}-{profile['speaker']}-{profile['name']}",
...
def create_direct_tts_audio(path, text, voice_profile, seed):
    ...
    profile = {
        ...
        "name": f"{WORK_TITLE}-{voice_profile['speaker']}-{voice_profile['name']}",
        "voice_type": "reference",
        "language": "zh",
        "default_engine": VOICE_REFERENCE_ENGINE,
        "default_model_size": VOICE_REFERENCE_MODEL_SIZE,
    ...
    for asset_name in [scene, *assets]:
        ...
        image_assets.append({
            ...
            "aliases": unique_nonempty([
                ...
                *CHAPTER_VIDEO_REFERENCE_BINDING_ALIASES.get(asset_name, []),
            ]),
            "identityAnchors": item.get("identityAnchors"),
            "negativePrompt": item.get("negativePrompt"),
            "views": list(item.get("views") or []),
...
def build_shot_continuity_prompt(state):
    ...
    scene_palette = gongbi_v2.render_prompt_safe_scene_palette(state.get("palette") or "")
...
def build_visual_continuity_fingerprint(prompt, manifest, state):
    ...
    return stable_json({
        ...
        "styleContract": {
            "version": gongbi_v2.STYLE_CONTRACT_VERSION,
            "fingerprint": gongbi_v2.style_contract_fingerprint(style_reference_sha256 or None),
            "promptAuditVersion": gongbi_v2.PROMPT_AUDIT_VERSION,
...
def main():
    ...
    for scene_no, track_key in enumerate(track_keys, 1):
        ...
        scene_output = EXPORTS / f"{WORK_TITLE}_EP01_scene_{scene_no:02d}.mp4"
    ...
    if not SKIP_PROJECT_WRITE:
        ...
        save_json(SCRIPT_JSON, {
            ...
            "scriptData": {"episodeId": EPISODE_ID, "title": EPISODE_DISPLAY_TITLE, "source": "latest scriptDraft"},
            "shots": storyboards,
            "parseStatus": "success",
            "updatedAt": now,
        })
...
def run_cli(argv=None):
    parser = argparse.ArgumentParser(
        description="Generate the current approved ChapterVideo chapter-001 workflow.",
    )
    parser.add_argument(
        "--run",
        action="store_true",
