...
const automationScript = readFileSync(`${appsRoot}/build/chapter_video/automate-chapter001-video.mjs`, "utf8");

function runHelper(payload: Record<string, unknown>, frozen: boolean) {
  return spawnSync("node", ["build/chapter_video/generate-storyboard-image.mjs"], {
    cwd: appsRoot,
...
function runAutomation(args: string[], env: Record<string, string> = {}) {
  return spawnSync("node", ["build/chapter_video/automate-chapter001-video.mjs", ...args], {
    cwd: appsRoot,
...
describe("ChapterVideo image-generation freeze guard", () => {
  ...
  it.each([
    ...
    expect(result.status).not.toBe(0);
    expect(output).toContain(`MYSTUDIO_IMAGE_GENERATION_FROZEN=1: blocked ChapterVideo image generation mode=${mode}`);
  ...
  it("fails closed before visual preflight for the default chapter command while frozen", () => {
    ...
    const report = JSON.parse(
      readFileSync(`${appsRoot}/output/automation/chapter001-video-report.json`, "utf8"),
    );

    expect(result.status).not.toBe(0);
    expect(output).toContain("MYSTUDIO_IMAGE_GENERATION_FROZEN=1: blocked ChapterVideo image generation mode=chapter001-video");
    expect(output).toContain("no provider request was sent");
  ...
  it("retains dry-run bypasses for continuity modes without executing them", () => {
    ...
    expect(automationScript).toContain("assertImageGenerationNotFrozen('continuity-asset-candidate', {");
    expect(automationScript).toContain("assertImageGenerationNotFrozen('continuity-pilot', {");
    expect(automationScript).toContain("assertImageGenerationNotFrozen('continuity-full-chapter', {");
  });
});
