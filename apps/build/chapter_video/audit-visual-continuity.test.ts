...
import {
  auditVisualContinuityState,
  isVisualPreflightEnabled,
} from "./audit-visual-continuity";
...
describe("ChapterVideo direct-video visual preflight", () => {
  it("requires an explicit environment flag before executing the import-safe audit module", () => {
    expect(isVisualPreflightEnabled({})).toBe(false);
    expect(isVisualPreflightEnabled({
      MYSTUDIO_CHAPTER_VIDEO_VISUAL_PREFLIGHT: "1",
    })).toBe(true);
  });

  it("accepts only current human-approved storyboards and asset versions", () => {
    const { scene, storyboard } = approvedFixture();
    expect(auditVisualContinuityState({
