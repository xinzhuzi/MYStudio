#!/usr/bin/env node
/**
 * 装机终验·出图补位 E2E(0924;17002 手动引擎 + headless Chrome 真前端口径):
...
async function main() {
  ...
  check("[40] 型选择=道具(1024²;装机应用链路被外部 TERM 三连,此跑压缩窗口)", String(setType).endsWith("道具"), String(setType));
  await sleep(1200);
  ...
  const sig = (blob) => blob.includes('"base":"道具"');
  ...
  if (hist.error) {
    ...
    check("sips 尺寸对账(道具型 1024x1024;维差≤16px·比例≤1%)",
    ...
    check("服务端排队图 base=道具(history prompt[2])", String(hist.promptBlob || "").includes('"base":"道具"'), "history prompt[2] 内 base 字段");
    report.cases.t2i = { imgName: saveImg.filename, destPath, bytes: buf.length, size, secs, pid: hist.pid };
  }
  } // ← PE_ONLY 守卫收口

  // ── edit PE-I2I 开路(排队通过即证+interrupt 复位) ──
  ...
  await waitFor(() => page.ev(vis(`window.app.graph && window.app.graph._nodes.length === ${edit.nodes.length} ? ${edit.nodes.length} : null`)),
    { timeout: 40_000, interval: 1000, label: `画布切换(${edit.nodes.length} 节点)` });
  await sleep(1500);
  const swOn = await page.ev(`(() => {
    const ns = window.app.graph._nodes.filter(n => n.type === 'ComfySwitchNode');
    if (!ns.length) return 'no-switch-nodes';
    const n = ns.find(n => String(n.id) === '15') || ns[0];
  ...
  check("PE 开: edit [15] ComfySwitch=true", String(swOn).endsWith("=true") && String(swOn).includes("id15"), String(swOn));
  await sleep(1000);
  const drySW = await page.ev(`(async () => {
    try { const p = await window.app.graphToPrompt();
      const out = [];
      for (const k of Object.keys(p.output || {}).sort()) if (p.output[k].class_type === 'ComfySwitchNode') out.push({ node: k, switch: p.output[k].inputs.switch });
      return JSON.stringify(out); } catch (e) { return 'err:' + e.message; }
  })()`);
  check("PE 开: 干跑排队图 ComfySwitchNode switch=true", String(drySW).includes('"switch":true'), String(drySW).slice(0, 300));

  const knownPidsPE = new Set(Object.keys(await (await fetch(`${ENGINE}/history`)).json()));
  await sleep(500);
  const queuedPE = await page.ev(`(async () => {
    const app = window.app;
    try { await app.queuePrompt(); return 'queued'; } catch (e) { return 'err:' + (e && (e.message || e)); }
  })()`);
  ...
  if (peBlob) {
    ...
    const hasSwTrue = /"15"\s*:\s*\{\s*"inputs"\s*:\s*\{\s*"switch"\s*:\s*true/.test(peBlob);
    check("PE 排队图: 服务端新 pid 含 pe_i2i 权重+TextGenerate+[15]switch=true",
      hasPEWeight && hasTextGen && hasSwTrue,
      `pid=${String(pePid).slice(0, 8)};pe_i2i权重=${hasPEWeight};TextGenerate=${hasTextGen};[15]switch=true=${hasSwTrue}`);
    peEvidence.pid = pePid;
    peEvidence.queueBlobHas = { peWeight: hasPEWeight, textGenerate: hasTextGen, switchTrue: hasSwTrue };
    peEvidence.queueBlobSnippet = peBlob.slice(0, 1500);
  } else {
    check("PE 排队图: 服务端新 pid 含 pe_i2i 权重+TextGenerate+[15]switch=true", false, "120s 内未在 /queue+/history 捕到新 pid 排队图");
  }
  try {
  ...
  writeFileSync(join(E2E_DIR, "accept-17002-report.json"), JSON.stringify(report, null, 2));
  page.close();
  killChrome();
  log("════ 汇总 ════");
...
main().catch((e) => {
  ...
  try { writeFileSync(join(E2E_DIR, "accept-17002-report.json"), JSON.stringify({ fatal: String(e.message), results, consoleErrors }, null, 2)); } catch { /* best effort */ }
  killChrome();
  process.exit(1);
});
