...
SUBJECTS = {
    ...
    "三视图": "同一位玄色劲装的青年刀修全身三视图：正面、侧面、背面三格同以自然站姿垂立，双臂拢袖自然下垂，腰侧佩刀不持握；三格头顶至脚底完整对齐，头身与肩宽跨格一致，服饰纹样、配饰与发型跨格完全一致。",
    ...
    "表情差分": "同一位红衣女修的九宫格表情差分：沉静垂目、含笑不语、怒目剑眉、哀恸闭眼、惧色微颤、凌厉逼视、惊讶抬眉、害羞侧首、决然睁眼；三行三列共九格，各格头部角度与光源方向保持一致，只有神态与眉眼唇形变化。",
