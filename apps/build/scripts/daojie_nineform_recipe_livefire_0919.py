...
SUBJECTS = {
    ...
    "三视图": "同一位玄色劲装的青年刀修三视图资产板：纯白无色背景、无任何场景元素；横幅并排恰四格等分——第一格上半身特写，画面底缘止于腰部、腰部以下不入画；第二格正面全身；第三格标准正侧面全身，严格九十度侧、不是四分之三侧；第四格纯背面全身；三格全身同以自然站姿、双臂拢袖自然下垂，腰侧佩刀不持握；头顶至脚底跨格对齐，头身肩宽一致，面容发型服装肤色跨格完全一致；恰四格，无多余人物，资产参考图，非海报。",
