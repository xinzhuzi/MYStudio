...
SUBJECTS = {
    ...
    "三视图": "同一位玄色劲装的青年刀修三视图资产板：纯白无色背景、无任何场景元素；横幅并排恰六格等分，从左到右依次为：第一格上半身特写（画面底缘止于腰部、腰部以下不入画），第二格正面全身，第三格标准正侧面全身（严格九十度侧、只见单侧轮廓），第四格纯背面全身（完全背对、非侧背），第五格四分之三背侧全身（回首回望），第六格四分之三斜侧全身；恰六格、每格一人一形态、互不重复、无第七格；各全身格同以自然站姿、双臂拢袖自然下垂，腰侧佩刀不持握；头顶至脚底跨格对齐，头身肩宽一致，面容发型服装肤色跨格完全一致；无重复形态、无无关个体，资产参考图，非海报。",
