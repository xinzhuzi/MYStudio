...
生成 docs/prompts/Qwen-Image-2.1/05-道劫规范提示词库.md:
  ...
  每型一节(标题「### {zh}-基础」),恰 9 节。四层装配逐层成文:
    ①主体句槽 ⟨①:…⟩(预置例一;每型 2 条示例)
    ②型底座·美化版(09-23 美化口径:按《三国望神州》v2.2+手册 prefix.md 词汇把 canon
      positive 的锁质要点写成更有画意的成文——纯画法零物象骨不改、每型锁质要点逐项
      保留、只用手册/圣经原生词汇(提按顿挫/墨分五阶/细稳笔性/薄透罩染/疏朗呼吸…),
      禁自造质感词、禁质量词;本层即 ②层唯一真源,canon-json 逐字锚废止)
    ③通用锁层(prefix.md §四.1/.2/.8 三段全文;人物系六型再并 §四.4-.7 四把全员锁;
      提取只去「（硬,…)」类元语言括注与 markdown 符号,正文一字不动——美化轮零触碰)
    ④配色职责行(prefix.md 配色五职责节「精选方案示例」对应型原文一行)
  库首附完整通用锁层常量(基础常量 + 人物系增量常量,供工作流恒挂层直接取用);
  文末覆盖矩阵(canon 18 骨干 + 八锁 × 九型,字面在场核验按美化版底座重算,未覆盖项如实标注)
  + 禁混条款五则 + 自查记录。

真源链(09-23 美化版起):本库 ②层(美化版底座)→ 工作流 qi21-道劫-t2i.json 装配子图常量
  逐字=本库 → 契约测试库↔工作流互锁(原 daojie_bases.json positive 逐字锚废止,型名/顺序
...
# 提按顿挫/墨色浓淡(五阶)/淡墨远景/近处轮廓/底色/传统色强度/点题色/均匀柔光平涂);
# 词汇只用《三国望神州》v2.2+手册 prefix.md 原生词(提按顿挫/墨分五阶/虚实/疏朗/呼吸/
# 细稳/笔性/薄透罩染/画册/连环画…);全角标点;无质量词/无禁句式/无自造质感词。
BEAUTIFIED = {
    "人物": (
        "主体的单人立绘，全身入画。运笔有提按顿挫的细墨线勾勒全身轮廓，线随结构时粗时细，"
        "转折衔接处轻重分明；墨色浓淡分明，干湿五阶层次清楚，近处轮廓清楚、墨线饱满，"
        "远景以淡墨晕染层层退开。大面积素净的暖白底色上，传统色中等强度铺陈，"
        "只留一小块鲜明的点题色收束视线；均匀柔光，浅净平涂的底，画面疏朗有呼吸。"
    ),
    "场景": (
        "空镜场景，前景、中景、远景三层分明。近处以运笔有提按顿挫的细墨线勾勒，线随结构时粗时细；"
        "中景用色块晕开，墨与色相互接晕；远景交给淡墨，墨色浓淡分明，层层退远、渐淡渐虚。"
        "大面积留白承担画面呼吸，底色浅净，只留一小块鲜明的点题色；均匀柔光，平涂的底，颜色清透。"
    ),
    "道具": (
        "主体的多视图设定板，各方位单独成格，摆在浅净底上。运笔有提按顿挫的细墨线勾勒轮廓，"
        "线随结构时粗时细，转折处轻重分明，细稳处带手绘笔性；墨色浓淡分明。"
        "传统色只占一小块，薄透罩染轻敷；均匀柔光，平涂的底。"
    ),
    "美宣": (
        "主体落在视觉焦点，背景用淡墨退去，笔墨比主体更简。运笔有提按顿挫的细墨线勾勒主体轮廓，"
        "线随结构时粗时细；墨色浓淡分明，焦点处墨线饱满，远景淡化为一片淡墨的虚实。"
        "大面积素净的暖白底色上，传统色中等强度铺陈，一小块鲜明的点题色收束视线；均匀柔光，平涂的底。"
    ),
    "三视图": (
        "同一人的转面设定板，多格并置如画册。运笔有提按顿挫的细墨线勾勒，线随结构时粗时细，"
        "各格笔性一致；墨色浓淡分明，墨线细稳。传统色素净，只占一小块，薄透罩染轻敷；"
        "浅净平涂底，均匀柔光，画面疏朗安静。"
    ),
    "高清人脸": (
        "主体的头像特写，居于画面中心。运笔有提按顿挫的细墨线勾勒轮廓，线随结构时粗时细，"
        "细稳处带手绘笔性；墨色浓淡分明。传统色微微染开，色彩匀净，薄透罩染轻敷；纯色平涂底，均匀柔光。"
    ),
    "分镜剧情图": (
        "一幅叙事画面，笔意连贯如连环画。前景、中景、远景层层分开：近处以运笔有提按顿挫的细墨线勾勒，"
        "线随结构时粗时细；远景用淡墨退去，墨色浓淡分明，渐远渐虚。大面积素净的暖白底色上，"
        "传统色中等强度铺陈，一小块点题色点亮叙事；均匀柔光，平涂的底。"
    ),
    "表情差分": (
        "一幅三行三列的九宫格，每格是同一个人的一种表情，半身入画。运笔有提按顿挫的细墨线勾勒，"
        "线随结构时粗时细，九格笔性如一；墨色浓淡分明。同一人跨格完全相同，各格底色相同；"
        "均匀柔光，平涂的底，画面疏朗安静。"
    ),
    "概念气氛图": (
        "一幅以气氛为主的画面，主体落在画面偏侧。细墨线只勾近处轮廓，运笔有提按顿挫，线随结构时粗时细；"
        "墨色浓淡分明，远处交给淡墨，层层退远，整体对比低。大面积留白承担画面呼吸，远景轻轻压住，"
        "近处轮廓清楚而细节少；底色浅净，只留一小块鲜明的点题色；均匀柔光，平涂的底。"
    ),
}
...
SECTION_PROSE = {
    ...
    "表情差分": dict(
        scene="同角色表情差分资产——生产路=单表情单发+后期拼板;九宫格直出仅快览(②层底座=快览版式,1:1 九宫格上限 9,>9 走每表情整图流)。",
...
# ── 通用锁层提取(与 research/09-jia_assemble.py 同则;美化轮零触碰)─────────
...
def build_layers(bases_json: list, md: str) -> dict:
    """②层=BEAUTIFIED 美化版(09-23 口径);aspect/MP/palette 仍取 canon+手册。"""
...
def assembly(zh: str, layers: dict, subject: str) -> str:
    return "\n".join([f"⟨①:{subject}⟩", layers[zh]["base"], *layers[zh]["locks"], layers[zh]["pal"]])


# ── 覆盖矩阵(字面在场核验,按 09-23 美化版底座重算;变体/不适用如实标注)──────

G = [
    # (num, 骨干名, 一级判据(任一子串中=✔), 变体判据(任一中=◐ 型内变体);均未中→NOTE 表 ✗/—)
    ("1", "细墨线勾勒(细线硬锁)", ["细墨线勾勒", "细墨线只勾近处轮廓"], []),
    ("2", "细稳+笔性(带手绘笔性)", ["细稳", "手绘笔性"], ["笔性"]),
    ("3", "提按顿挫/转折轻重", ["提按顿挫", "转折处轻重提按"], ["轻重分明"]),
    ("4", "连续铁线描,线宽连续有节奏", ["连续铁线描", "线宽连续且有节奏"], []),
    ("5", "墨色浓淡分明(五阶)", ["墨色浓淡分明"], ["墨分五阶", "浓淡干湿"]),
    ("6", "淡墨晕开远景/层层退远/远景淡化",
     ["淡墨晕染", "淡墨层层退远", "远景用淡墨退去", "背景用淡墨退去", "远景淡化"],
     ["层层退远", "远处交给淡墨"]),
    ("7", "近处轮廓清楚(而细节少)", ["近处轮廓清楚"], []),
    ("8", "大面积素净暖白底/底色浅净", ["暖白底色", "底色浅净"],
     ["浅净平涂底", "纯色平涂底", "摆在浅净底上", "各格底色相同"]),
    ("9", "墨与色落在平涂色场上", ["墨与色落在平涂色场上"], []),
    ("10", "传统色中等强度", ["传统色中等强度"],
     ["颜色清透", "传统色只占一小块", "传统色素净", "传统色微微染开", "对比低"]),
    ("11", "一小块鲜明的点题色", ["一小块鲜明的点题色"],
     ["一小块点题色", "传统色只占一小块", "只占一小块"]),
    ("12", "矿物罩染薄透/颜色清透", ["薄透矿物罩染"], ["颜色清透", "薄透罩染"]),
    ("13", "均匀柔光,平涂的底", ["均匀柔光", "平涂的底"], ["浅净平涂底", "纯色平涂底"]),
    ("14", "前/中/远三层分开", ["前、中、远三层", "前景、中景、远景"], ["前景、中景、远景层层分开"]),
    ("15", "中景用色块晕开(场景)", ["中景用色块晕开"], []),
    ("16", "现代修仙游戏的数字绘画资产(第一眼)", ["现代修仙游戏的数字绘画资产"], []),
    ("17", "留白承担(画面/视觉呼吸)", ["留白承担画面", "留白承担视觉呼吸"], ["承担画面呼吸"]),
    ("18", "画面干净可读/生产级清晰度", ["表面干净可读", "生产级最终画面清晰度"], []),
]

_SHE_DING = "设定板/格子页版式"  # 道具/三视图/高清人脸/表情差分
NOTE = {
    ...
    ("7", "场景"): "✗ 型底座无此句;近处由「细墨线勾勒」承担线法(美化版同判)",
    ("7", "美宣"): "✗ 型底座无此句(「背景用淡墨退去,笔墨比主体更简」承担近简关系)",
    ("7", "分镜剧情图"): "✗ 型底座无此句(「远景用淡墨退去」承担远近关系)",
    ...
    ("17", "人物"): "✗ 型底座与通用锁层均无「留白承担」字样;暖白底色+疏朗有呼吸近义承担(美化版同判)",
    ("17", "美宣"): "✗ 同人物判:暖白底色近义承担",
    ("17", "道具"): "✗ 同人物判:浅净底近义承担",
    ("17", "三视图"): "✗ 同人物判:浅净平涂底+疏朗安静近义承担",
    ("17", "高清人脸"): "✗ 同人物判:纯色平涂底近义承担",
    ("17", "分镜剧情图"): "✗ 同人物判:暖白底色近义承担",
    ("17", "表情差分"): "✗ 同人物判:各格底色相同+疏朗安静近义承担",
}
...
def backbone_mark(num: str, zh: str, full: str) -> str:
    _, _, primary, variants = _GM[num]
    if any(s in full for s in primary):
        return "✔"
    for tok in variants:
        if tok in full:
            return f"◐ 变体句在场:{tok}"
...
def matrix_tables(zh_order: list, layers: dict) -> tuple[list, list]:
    rows18 = []
    for num, name, _, _ in G:
        cells = []
        for zh in zh_order:
            full234 = "\n".join([layers[zh]["base"], *layers[zh]["locks"], layers[zh]["pal"]])
...
def build_doc() -> str:
    ...
    L = []
    A = L.append
    A("# 05 · 道劫规范提示词库(canon 九型·甲案全中文,四层装配;②层底座 09-23 美化版)")
    A("")
    ...
    A("- 真源四件:①型录/画幅=`daojie_bases.json`(九型 zh 顺序,aspect_ratio/megapixels 字段);②层底座=本库(09-23 美化版,唯一真源);③通用锁层/配色/留白分档/连环画媒介层=手册 `prefix.md`(仓库副本 `apps/frontend/assets/studio-manuals/art_skills/daojie_ink_guofeng/prefix.md`,与装机手册家 diff 逐字节一致,09-23 本轮核验);④主体句示例=`docs/prompts/道劫_九型主体句示例.md`;风格真源与背景=research/06(锁质清单与旧 16 条偏差对照)、07(手册本体 vs 旧 16 条深度对比)、08(LoRA 搜集:Q2.1 水墨工笔 LoRA 生态零件,画风增益全靠底座词法)。")
    A("- 旧库处置:本文件前版(16 条英文长文,人物/场景/道具/美宣各 4)已随甲案重写**整体退役**——其宪法结构达标但手册锁质缺失/降格,六大缺陷模式逐条点名见 research/06 §3.1;Q2.1 侧 PE 改写路([14] true)不受本库影响,仍走官方宪法。")
    A("")
    A("---")
    A("")
    A("## 一、用法(四层装配·一处换句)")
    A("")
    A("**装配顺序**(照甲案;③④原文照抄,②=本库美化版):")
    A("")
    A("```text")
    A("[①主体句·题材正文](七段公式简写,短——本库 ⟨①:…⟩ 槽,唯一手写位)")
    A("+ [②型底座·美化版纯画法锁质](本库该型节②层成文;型名/画幅档对齐 daojie_bases.json)")
    ...
    A("3. **贴画布**:`qi21-道劫-t2i.json`(唯一用法,09-23 午定谳——前代道劫直写件已随轮退役删除)把「九型底座+通用锁层+四层装配」收进 **[40] 装配子图**(双击进入=九型底座常量+八级选型级联+锁层恒挂+换行拼接+PE/RGBA 路由):换型=子图面板八级开关一处切换(不跨型拼装),主体句=[24] 外露槽,装配预览=[27];其底座/锁层常量逐字=本库(生成器 `apps/build/scripts/qi21_daojie_t2i_0923.py` 从本库现读,契约测试互锁)。")
    A("")
    A("**长度口径**:手册长度契约 300-800 字符(prefix.md:230)约束的是 ma-gongbi-v1 编译器装配的 LLM 生成路;本库直写槽路无编译器长度门,按 round5-jia 实拍口径(装配全文 1475-2086 字符含换行)整段注入,Q2.1 直写槽不截断(实拍 /history 逐字核验)。美化版②层较 canon 略长(每型+40-80 字符),仍在同量级。")
    A("")
    ...
    A("")
    A("**表情差分两路**:生产路=**单表情单发+后期拼板**(09-20 裁定13+活模板 daojie_emotion_compose_0920.py,每表情独立整图再拼);九宫格直出仅快览——本库②层底座=快览版式,单发时主体句只写一格的半身与该表情五官全覆盖描述(见该型节)。")
    A("")
    ...
    A("2. **K2 装配结构与 LoRA 栈假设禁入**。K2 道劫=「底座在前+[50] 主体句零分隔符直拼」(道劫_底座节点_0918.md:4)且依赖九型 LoRA 配方;本库甲案=**主体句领头**+②③④层**换行分层**(round5-jia 实拍注入即此式),Q2.1 侧水墨工笔 LoRA 生态零件(research/08)——画风增益全靠四层词法。按 K2 词序拼装、零分隔符直拼、或把 K2 LoRA 配方当本库前提=违规。②层唯一真源=本库(09-23 美化版);daojie_bases.json 与 K2 共享的 canon positive 保留为 K2 侧取材,不再是本库②层逐字源(型名/顺序/画幅档仍对齐)。")
    ...
    A("> 未覆盖项汇总(如实,美化版口径):骨干 5(墨色浓淡分明)已九型全在场——旧 canon 口径在道具/三视图/高清人脸/表情差分四型字面缺席,美化版以「墨色浓淡分明/墨分五阶」成文补齐;骨干 17(留白承担)在场景/概念气氛图之外七型仍字面缺席(暖白/浅净底+疏朗近义承担);骨干 10/11 的「传统色中等强度/一小块鲜明的点题色」在设定板/特写/格子页诸型以变体句或让渡①④层承担;骨干 7 在设定板四型不适用。以上均为美化版②层成文的型内事实;改②层先改本脚本 BEAUTIFIED 并重跑(真源=本库②层,09-23 美化版口径)。")
    A("")
    A("**自查记录(生成器实跑)**")
    A("")
    chars = []
    ...
    A("- 围栏配对、### 计数、②③④逐字对齐:`--check` 实跑全绿(校验内容=文件 ### 标题恰为九型『### {型}-基础』序列、每 ```text 围栏闭合、每型装配全文去①槽行后与 ②美化版底座+③锁层+④配色行 逐字节相等、库首两常量与 prefix.md 提取逐字节相等)。".replace("{型}", "型"))
    A("- 主体句重叠预检(18 条示例 × 各自型美化版底座,判据 ≥10 字公共子串,生成器逐条实跑):**18/18 零重复**。")
    A("")
    A("## 六、演进与待裁定(09-23 深检吸收轮立账;零围栏零 ###,重跑不漂)")
    A("")
    A("- **PE 默认值 A/B 对拍后再定(待办)**:qi21-道劫-t2i [140] QwenImage21_T2IPromptRewrite 现默认=插件官方 README 推荐值(temp1.0/topP0.95/topK20/presence1.5/max16256),与视频侧 ComfyUI 官方模板值(temp0.7/topK64/presence0.00/max4096)不一致属待实测分歧非错误(research/10 §2.3/§5.2)——排一期同 seed 同短句 A/B 对拍,比对 rewritten_prompt 质量与耗时后再定夺,未定前勿改画布默认。")
    A("- **④配色行 hex 色值锚(低优先研究)**:为④配色行补标准 hex 色值锚(须色彩学真源,禁自造),对齐官方「逐字逐值」法(官方 s047/s104 hex 色卡例,research/11 §三/§四.1)。")
    A("- **空格/裂格应急句字典(备而不发)**:不改 v2.1『砍计数断言』现行规范;仅当表情差分九宫格/格子类实测出现空格裂格时,第一应急件借官方肯定式占格句——「每一格里都必须画出角色本人…约占该格高度 2/3 以上」/「输出为一张扁平整图,N 格并排合并」(research/12 答A备2;我方像素带检测验收口径=09-20 裁定21 实证,先验收再动用)。")
    A("- **待用户裁定·压文字句**:「画面中无任何可识别的文字信息」是否加入③层成片质量锁或①层句尾以压水墨出图偶发印章/题字漂移——传统画要不要印章属美术口径,手册未裁定,接线师不得自行加入(官方 3.1.1 用此肯定式压文字,research/12 答A待裁定4)。")
    A("")
    return "\n".join(L) + "\n"
...
def check() -> int:
    ...
    print(f"✅ 守恒校验全绿:### 恰 {len(headings)}(九型『型-基础』节,型名/顺序与 canon 逐字对齐);"
          f"``` 围栏 {len(fences)} 行({len(fences)//2} 对)全配对;九型 ②③④ 层与真源逐字一致(②=09-23 美化版);常量 A/B 与手册提取逐字一致。")
    return 0
    ...
    for zh, (s1, s2, _) in SUBJECTS.items():
        for tag, s in (("例一", s1), ("例二", s2)):
            ...
            for clause in re.split(r"[；。]", s):
                ...
                if len(clause) < 10:
                    continue
                m = difflib.SequenceMatcher(None, clause, BEAUTIFIED[zh]).find_longest_match(0, len(clause), 0, len(BEAUTIFIED[zh]))
                if m.size >= 10:
