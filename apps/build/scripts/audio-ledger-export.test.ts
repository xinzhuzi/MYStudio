  ...
  const manifest = await makeSealedManifest([
    {
      ...
      index: 1,
      durationUs: 5120000,
      audioBindings: [binding, ...(options.extraBindings ?? [])],
    },
  ]);
  const audioSha = sha256("fake-audio-001");
...
describe("buildChapterLedger", () => {
  ...
  it("shot-sfx 封印被篡改 → 纳入失败汇总且 allChecksPass=false", async () => {
    const sfxBytes = "shot-sfx-tamper";
    const sfxSha = sha256(sfxBytes);
    const sfxBinding: Record<string, unknown> = {
      schemaVersion: 2,
      bindingId: "sfx:sb-chapter-001-001:tamper",
      bindingFingerprint: "",
      source: {
        kind: "project-file",
        relativePath: `remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/sfx/${sfxSha}.wav`,
        contentSha256: sfxSha,
      },
      sourceFingerprint: sfxSha,
      durationUs: 200_000,
      volume: 1,
      role: "sfx",
      renderScope: "shot",
    };
    sfxBinding.bindingFingerprint = await computeBindingFingerprint(sfxBinding);
    sfxBinding.volume = 0.25;
    const { ledger } = await buildFixture({
      extraBindings: [sfxBinding],
      files: {
        [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/voice/${sha256("fake-audio-001")}.wav`]: "fake-audio-001",
        [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/sfx/${sfxSha}.wav`]: sfxBytes,
      },
    });
    expect(ledger.shots[0]!.shotSfx).toHaveLength(1);
    expect(ledger.shots[0]!.shotSfx![0]!.bindingSeal).toBe(false);
    expect(ledger.summary.bindingSealFail).toBe(1);
    expect(ledger.summary.allChecksPass).toBe(false);
  });
  ...
  it("时间轴优先使用 editing project,并记录 duration mismatch", async () => {
    const audioSha = sha256("fake-audio-001");
    const files = {
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/voice/${audioSha}.wav`]: "fake-audio-001",
    };
    const baseIo = makeIo(files);
    const io: TtsLedgerIo = {
      ...baseIo,
      readProjectJson(relativePath) {
        expect(relativePath).toBe(`remotion/jobs/chapter/${CHAPTER_ID}/current-editing-project.json`);
        return {
          clips: [{ id: "visual-sb-chapter-001-001", startUs: 123_000, durationUs: 9_000_000 }],
        };
      },
    };
    const { ledger } = await buildFixture({ files, io });
    expect(ledger.timelineAnchor).toEqual({ source: "editing-project", mismatch: true });
    expect(ledger.shots[0]!.shotStartUs).toBe(123_000);
    expect(ledger.shots[0]!.durationUs).toBe(9_000_000);
  });

  it("editing project 缺失时回退 manifest 累计时间轴并标记 mismatch", async () => {
    const audioSha = sha256("fake-audio-001");
    const files = {
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/voice/${audioSha}.wav`]: "fake-audio-001",
    };
    const baseIo = makeIo(files);
    const io: TtsLedgerIo = { ...baseIo, readProjectJson: () => null };
    const { ledger } = await buildFixture({ files, io });
    expect(ledger.timelineAnchor).toEqual({ source: "manifest-cumulative", mismatch: undefined });
    expect(ledger.shots[0]!.shotStartUs).toBe(0);
    expect(ledger.shots[0]!.durationUs).toBe(5_120_000);
  });

  it("全角色音频、章级叠加、music 对拍和 orphan 分桶均显式登记", async () => {
    const voiceSha = sha256("fake-audio-001");
    const sfxBytes = "shot-sfx";
    const sfxSha = sha256(sfxBytes);
    const bgmBytes = "shared-bgm";
    const bgmSha = sha256(bgmBytes);
    const sfxBinding: Record<string, unknown> = {
      schemaVersion: 2,
      bindingId: "sfx:sb-chapter-001-001:one",
      bindingFingerprint: "",
      source: {
        kind: "project-file",
        relativePath: `remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/sfx/${sfxSha}.wav`,
    ...
    sfxBinding.bindingFingerprint = await computeBindingFingerprint(sfxBinding);
    const sharedBinding: Record<string, unknown> = {
      schemaVersion: 2,
      bindingId: "bgm-chapter-001",
      bindingFingerprint: "",
      source: {
        kind: "project-file",
        relativePath: `remotion/audio/${CHAPTER_ID}/shared/bgm/${bgmSha}.ogg`,
        contentSha256: bgmSha,
      },
      sourceFingerprint: bgmSha,
      chapterStartUs: 0,
    ...
    const voiceBinding = await makeSealedBinding("sb-chapter-001-001");
    const manifest = await makeSealedManifest([{
      shotId: "sb-chapter-001-001",
      revision: 4,
      durationUs: 5_120_000,
      audioBindings: [voiceBinding, sfxBinding],
    }]);
    manifest.sharedAudioBindings = [sharedBinding];
    manifest.manifestFingerprint = await computeManifestFingerprint(manifest);
    const files = {
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/voice/${voiceSha}.wav`]: "fake-audio-001",
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/voice/${sha256("old-voice")}.wav`]: "old-voice",
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/sfx/${sfxSha}.wav`]: sfxBytes,
      [`remotion/audio/${CHAPTER_ID}/shots/sb-chapter-001-001/sfx/orphan.wav`]: "orphan-sfx",
      [`remotion/audio/${CHAPTER_ID}/shared/bgm/${bgmSha}.ogg`]: bgmBytes,
      [`remotion/audio/${CHAPTER_ID}/shared/ambience/orphan.wav`]: "orphan-ambience",
      [`remotion/audio/${CHAPTER_ID}/shared/sfx/orphan.wav`]: "orphan-chapter-sfx",
      "music/片头曲/op.wav": bgmBytes,
      "music/片头曲/歌词.md": "歌词",
    };
    const ledger = await buildChapterLedger({
      manifest,
    ...
    expect(ledger.summary.musicLibraryFiles).toBe(1);
    expect(ledger.musicLibrary[0]).toMatchObject({
      relativePath: "music/片头曲/op.wav",
      category: "片头曲",
      crossRefDocs: ["music/片头曲/歌词.md"],
  ...
  it("相切区间不计入 chapterOverlay", async () => {
    expect(overlapUs([0, 10], [10, 20])).toBe(0);
    expect(overlapUs([0, 10], [2, 8])).toBe(6);
    expect(overlapUs([0, 10], [-5, 5])).toBe(5);
    expect(overlapUs([0, 10], [20, 30])).toBe(0);
  });
});
...
describe("renderTtsLedgerMarkdown", () => {
  it("渲染摘要/逐镜表/指纹附录,失败项落异常明细", async () => {
    ...
    expect(markdown).toContain("# 全声音台账 · chapter-001");
