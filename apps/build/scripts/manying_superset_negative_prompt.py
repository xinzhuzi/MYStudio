...
def main() -> int:
    ...
    # 09-17 用户裁「都做」:换修缮句(负面台账 D2居中/D3双色倒挂/D4冷底;语句体+全角)
    nodes[50]["widgets_values"] = [
        "一位容色清丽、眉目如画的黑发少女侧身立于画面右侧三分之一处，面朝左方，"
        "身姿修长端雅；左侧与上方留出过半空白，画面左下角一枝疏梅淡影斜出。"
        "她内着石青色襦裙，肩臂间绕有一条大面积的赭石色披帛，垂落过腰，"
        "与石青裙身构成冷暖相衬的主调；衣纹以精谨线描勾勒，线条细而稳，流畅有韵律。"
        "底色是温润的暖米色，细洁素净，通幅宁静古雅。"
        # 09-17 用户实证:「宣纸」词=脏先验(做旧泛黄),正向禁用;G2/G7 A/B 去词后空白区更亮更净
    ]
