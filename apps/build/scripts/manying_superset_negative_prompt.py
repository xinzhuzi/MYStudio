...
def main() -> int:
    ...
    # 09-17 二次修(用户裁「组装职责分离」):[50]=纯主体句(人物/五官/形制/配色分配/场景/方位),
    # 构图原则/线描渲染/质感/收尾=底座([60]锚定)职责,不再重复
    nodes[50]["widgets_values"] = [
        "一位清冷的黑发少女侧身立于画面右侧三分之一处，面朝左方；"
        "细吊眉，单凤眼，眼尾细长微微上挑，垂眸含蓄，唇小而润，肤色瓷白；"
        "发梳双垂髻，簪一支素银珠花簪；身着唐制齐胸襦裙外罩大袖衫，"
        "裙为石青色，肩臂间绕一条大面积的赭石色披帛，垂落过腰，裙腰缀金线缠枝纹；"
        "画面左下角一枝疏梅淡影斜出，底色温润米白，纸面细洁素净。"
    ]
