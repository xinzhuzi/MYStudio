...
def verify_fixture(manifest_path: Path) -> dict[str, Any]:
    ...
    golden_root = (fixture_root / "golden").resolve()
    references_root = (fixture_root / "references").resolve()
    reference_count = 0
    for row in rows:
        golden = row.get("goldenImage") or {}
        golden_path = (golden_root / str(golden.get("relativePath") or "")).resolve()
        if not golden_path.is_relative_to(golden_root):
            raise RuntimeError(f"golden image path escapes fixture root for storyboard {row.get('index')}")
        ...
        for reference in references:
            reference_count += 1
            reference_path = (references_root / str(reference.get("relativePath") or "")).resolve()
            if not reference_path.is_relative_to(references_root):
                raise RuntimeError(f"reference image path escapes fixture root for storyboard {row.get('index')}")
