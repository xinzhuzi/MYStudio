...
本文说明当前 `工作流` 页面从小说原文到候选片段、最终成片的基础流程。更细的按钮、状态、弹窗和数据关系见 [工作流阶段操作手册](./WORKFLOW_STAGE_OPERATIONS.md)。小说导入、事件分析和策划编剧细节见 [小说导入与策划编剧操作参考](./WORKFLOW_NOVEL_SCRIPT_OPERATIONS.md)，剧本资产和 ProductionAgent 细节见 [剧本资产与 ProductionAgent 操作参考](./WORKFLOW_ASSET_GENERATION_OPERATIONS.md)，分镜面板与视频工作台细节见 [分镜面板与视频工作台操作参考](./WORKFLOW_STORYBOARD_EDITING_OPERATIONS.md)。安装、配置和排错入口见 [文档中心](./README.md)。
...
工作流页本身按阶段推进。每一章按当前阶段输入、生成和验收，完成一章后再进入下一章。整体重心参考 Toonflow 的 `策划 -> 编剧 -> 分镜 -> 出片`：ScriptAgent 负责故事骨架、改编策略和结构化剧本；ProductionAgent 负责导演规划、衍生资产、分镜表、分镜面板和分镜图；视频工作台负责候选视频和最终导出。
...
先选择视觉手册和导演手册。视觉手册决定画风、角色和场景资产的基础美术语言；导演手册决定镜头、构图、运镜和生产约束。后续策划编剧、导演计划、ProductionAgent、分镜面板和生图生视频都会读取这里的配置。
...
进入 `工作流 -> ProductionAgent`。

这一阶段用于把结构化剧本、角色、场景和道具转成可用于分镜与视频生成的制作资料。它对应 Toonflow ProductionAgent 的导演规划、衍生资产预划、衍生资产分析/生成、剧集圣经锁定，以及后续分镜表的前置准备。
...
ProductionAgent 之后不是直接出片，而是进入分镜面板：用导演计划和资产库构建分镜表，写入分镜面板，绑定角色/场景/道具，生成分镜图，并按需要处理角色音色和配音。
...
确认当前章节有剧本草稿。没有剧本时，`剧本资产` 无法提取角色、场景和道具。
