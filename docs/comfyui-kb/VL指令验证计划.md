...
> **历史验证计划（2026-09-20 适用范围复核）**：原计划未标完整日期，内容基于退役的独立 ComfyUI 项目与 Desktop 端口；其模型路径、17598 端口、部署和重启命令不适用于当前托管引擎。保留原实验设计与证据，不据此启动、改写或清理现行环境。当前入口见 [ComfyUI 引擎指南](../settings/COMFYUI_ENGINE_GUIDE.md)。
