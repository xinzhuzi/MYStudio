...
> **历史离线包说明（2026-09-20 核对）**：本文原包未标注完整制作日期；35G、9 个工作流、3946 风格和 15 个插件仅描述该包，不代表当前仓库或安装状态，本轮未在 Windows 复跑。当前 MYStudio 托管引擎请按 [ComfyUI 引擎指南](../settings/COMFYUI_ENGINE_GUIDE.md) 操作；不得用本包覆盖现行引擎、模型或用户工作流。
...
2. 装好重启 → 打开 `K2-文生图-超集.json` → 写提示词 → 生成(全链 LoRA 栈默认激活;日常稳妥用 `K2-文生图.json`,LoRA 栈默认旁路)
...
| 文件 | 用途 |
|------|------|
| K2-文生图-超集 | 文生图超集档,LoRA 栈默认激活(09-15 替代简版;稳妥档=K2-文生图) |
