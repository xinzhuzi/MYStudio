...
1. 进入 `设置 → 云端AI -> 模型服务`，添加供应商、Base URL、API Key 和模型列表。
2. 进入 `设置 → 云端AI`，在 `服务映射` 区域为文本、图片、视频、TTS、视觉理解等能力绑定模型。
3. 进入 `设置 → 云端AI -> Agent 配置`，为通用AI、剧本草稿、事件分析、分镜分析、视觉提示词润色等任务绑定模型。
4. 如果要使用本地 TTS、声音克隆或角色试听，进入 `设置 → 本地配置`，在 `Python 运行环境` 区块点击 `开始配置`。
...
- `设置 → 本地配置 -> Python 运行环境` 已完成。
- `设置 → 本地配置 -> TTS 运行时与模型` 可以启动本地 TTS。
- 角色已分配音色。
- 音频样本路径仍然存在。
