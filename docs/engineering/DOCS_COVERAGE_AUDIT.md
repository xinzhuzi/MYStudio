...
| 自媒体 | `apps/frontend/components/panels/self-media/` | 【缺口】暂无用户文档（工程边界见 [自媒体 / AiToEarn 集成边界](./self-media-aitoearn-integration.md)） |
...
| MCP 服务 | 【缺口】设置页现含「MCP 服务」页签，暂无文档 |
| 图片规格 | 【缺口】设置页现含「图片规格」页签，暂无文档 |
...
| ComfyUI 引擎层（engines/comfyui、manying_nodes、webview 桥、comfyui/ 家） | 用户侧：[ComfyUI 引擎指南](../settings/COMFYUI_ENGINE_GUIDE.md)、[本地模型页](../panels/LOCAL_MODELS_GUIDE.md)；工程侧：[定制代码地图](../comfyui-kb/定制代码地图.md)、[参数速查](../comfyui-kb/参数速查.md)、[常见故障排查](./TROUBLESHOOTING.md)（引擎排障节）、`.claude/knowledge/node-graph-architecture.md`、`.agents/skills/comfyui/machine.md` |
...
1. 工作流已覆盖 `小说导入/剧本策划`、`剧本资产管理/剧情产物生成` 和 `分镜表/剪辑工作台` 专题；工作流阶段、按钮或弹窗变化时同步对应专题。
...
5. 辅助工作台已被全屏 ComfyUI 工作区取代（2026-09-10）；旧三篇已标注过时。「本地模型」页（画布/配音室/漫影生图）与设置页 ComfyUI 引擎卡是当前最大文档缺口，建议单独成篇。
6. 时间线 renderer 已覆盖全局选择、Remotion Player/Headless Shell、FFmpeg 兼容入口、bundle/compositor 打包和 renderer-specific evidence；真实 Daojie 双链仍按 Remotion Trellis 任务的 AC11 状态单独报告。
