...
export function StudioAssetDetailDialog({
  ...
  const regenerationPrompt = useMemo(() => buildAssetRegenerationPrompt(asset), [asset]);
  ...
  const handleRegenerate = () => {
    ...
    setImagePrompt(regenerationPrompt);
    setImageResult(null);
    setActiveTab("freedom");
    onOpenChange(false);
    toast.success("已带入图片工作室，可直接重新出图");
  };
  ...
  const handleAddImage = async () => {
    ...
    const imageName = prompt("请输入图片名称（如：正面、侧面、战斗姿态）");
          ...
          <div className="studio-asset-detail-preview border-r border-border bg-muted/20 p-4">
                            ...
                            {idx > 0 || img.name !== "主图" ? (
                              <div className="flex gap-1">
                                <button
                                  ...
                                  onClick={() => handleRenameImage(img, idx)}
                                  title="重命名"
                                >
            ...
            {asset.type === "audio" ? (
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => copyText("口播词句", spokenText)}>
                  <Clipboard className="mr-2 h-3.5 w-3.5" />
                  复制口播词句
                </Button>
                <Button variant="outline" size="sm" onClick={() => copyText("标题", displayName)}>
                  <Clipboard className="mr-2 h-3.5 w-3.5" />
                  复制标题
                </Button>
              </div>
              ...
              <>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => copyText("出图提示词", regenerationPrompt)}>
                    <Clipboard className="mr-2 h-3.5 w-3.5" />
                    复制出图提示词
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleOpenSource}>
